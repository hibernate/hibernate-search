...
public enum MultiValueMode {
	...
	MIN {
		...
		protected int pick(SortedSetDocValues values) throws IOException {
			long maxOrd = Long.MAX_VALUE;
			for ( long ord = values.nextOrd(); ord != SortedSetDocValues.NO_MORE_ORDS; ord = values.nextOrd() ) {
				maxOrd = Math.min( maxOrd, ord );
			}
	...
	/**
	 * Return a {@link NumericDocValues} instance that can be used to sort
	 * documents with this mode and the provided values.When a document has
	 * no value, <code>missingValue</code> is returned.Allowed Modes: SUM,
	 * AVG, MEDIAN, MIN, MAX
	 *
	 * @param values
	 * @return
	...
	/**
	 * Return a {@link NumericDocValues} instance that can be used to sort
	 * root documents with this mode, the provided values and filters for
	 * root/inner documents.For every root document, the values of its inner
	 * documents will be aggregated.If none of the inner documents has a
	 * value, then <code>missingValue</code> is returned.Allowed Modes: SUM,
	 * AVG, MIN, MAX
	 *
	 * NOTE: Calling the returned instance on docs that are not root docs is
	 * illegal The returned instance can only be evaluate the current and
	 * upcoming docs
	 *
	 * @param values
	 * @param missingValue
	...
	/**
	 * Return a {@link NumericDoubleValues} instance that can be used to
	 * sort documents with this mode and the provided values.When a document
	 * has no value, <code>missingValue</code> is returned.Allowed Modes:
	...
	/**
	 * Return a {@link NumericDoubleValues} instance that can be used to
	 * sort root documents with this mode, the provided values and filters
	 * for root/inner documents.For every root document, the values of its
	 * inner documents will be aggregated.If none of the inner documents has
	 * a value, then <code>missingValue</code> is returned.Allowed Modes:
	 * SUM, AVG, MIN, MAX
	 *
	 * NOTE: Calling the returned instance on docs that are not root docs is
	 * illegal The returned instance can only be evaluate the current and
	...
	/**
	 * Return a {@link SortedDocValues} instance that can be used to sort
	 * documents with this mode and the provided values.Allowed Modes: MIN,
	 * MAX
	 *
	 * @param values
	 * @return
	 ...
	 * root documents with this mode, the provided values and filters for
	 * root/inner documents.For every root document, the values of its inner
	 * documents will be aggregated.Allowed Modes: MIN, MAX
	 *
	 * NOTE: Calling the returned instance on docs that are not root docs is
	 * illegal The returned instance can only be evaluate the current and
	 * upcoming docs
