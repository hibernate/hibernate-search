...
public class UpdateIndexedEmbeddedCollectionTest extends SearchTestCase {
	...
	/**
	 * This test case refers to HSEARCH-735 issue which prevents the NPE reported in HSEARCH-734 issue. 
	 * @throws Exception
	 */
	public void testUpdateIndexedEmbeddedCollectionWithNull() throws Exception {
		...
		// if the Hibernate-Search release is 3.4.0.CR2, it throws NullPointerException described in HSEARCH-734 during commit phase 
		tx.commit();
		session.close();
	...
	private Long findTruckIdFromIndex(FullTextSession session, String itemDescription) {
		FullTextQuery q = session.createFullTextQuery( new TermQuery( new Term( "truck.items.description", itemDescription ) ), Driver.class );
