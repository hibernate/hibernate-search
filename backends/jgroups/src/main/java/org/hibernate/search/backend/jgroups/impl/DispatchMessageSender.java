...
public final class DispatchMessageSender implements MessageSenderService {
	...
	public void send(final Message message, final boolean synchronous, final long timeout) throws Exception {
		...
		if ( synchronous ) {
			for ( Rsp rsp : rspList.values() ) {
				if ( !rsp.wasReceived() ) {
					if ( rsp.wasSuspected() ) {
						throw new SuspectedException( log.jgroupsSuspectingPeer( rsp.getSender().toString() ) );
					}
					else {
						throw new TimeoutException( log.jgroupsRpcTimeout( rsp.getSender().toString() ) );
					}
				}
				else {
					if ( rsp.hasException() ) {
						throw log.jgroupsRemoteException(
	...
	public void start(Properties properties, BuildContext context) {
		...
		channelContainer = buildChannelContainer( properties );
		...
		if ( handler instanceof Muxer ) {
			Short muxId = (Short) properties.get( MUX_ID );
			if ( muxId == null ) {
			...
			ClassLoader cl = (ClassLoader) properties.get( CLASSLOADER );
	 ...
	 */
	private static ChannelContainer buildChannelContainer(Properties properties) {
		final String clusterName = ConfigurationParseHelper.getString(
				properties, CLUSTER_NAME, DEFAULT_CLUSTER_NAME
		);
		if ( properties != null ) {
			final Object channelObject = properties.get( CHANNEL_INJECT );
			...
			final String cfg = properties.getProperty( CONFIGURATION_FILE );
			if ( cfg != null ) {
				try {
					log.startingJGroupsChannel( cfg );
					return new ManagedChannelContainer(
							new JChannel( ConfigurationParseHelper.locateConfig( cfg ) ),
							clusterName
					);
				}
				catch (Exception e) {
