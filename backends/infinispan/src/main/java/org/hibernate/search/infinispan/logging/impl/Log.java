...
public interface Log extends org.hibernate.search.util.logging.impl.Log {
	...
	@Message(id = INFINISPAN_BACKEND_MESSAGES_START_ID + 55, value = "Unable to retrieve CacheManager from JNDI [%s]")
	void unableToRetrieveCacheManagerFromJndi(String jndiNamespace, @Cause NamingException ne);

	@LogMessage(level = ERROR)
	@Message(id = INFINISPAN_BACKEND_MESSAGES_START_ID + 56, value = "Unable to release initial context")
