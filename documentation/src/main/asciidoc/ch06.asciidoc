...
working in parallel. The number of threads working on actual index writing is defined by the backend
configuration of each index. See the option `worker.thread_pool.size` in
