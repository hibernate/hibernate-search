...
beyond the index mapping which <<backend-elasticsearch-index-lifecycle,can be automatically generated>>.
...
You can also disable the query made to the Elasticsearch cluster which could be useful in some cases.
To do that you have to set the `hibernate.search.backends.<backend name>.version_check.enabled` to `false` but if you do
that you have to specify the version of Elasticsearch as we see just before with at least the major and minor version `x` and `y`.
