...
When indexing, the `__HSEARCH_type` field is populated transparently with type name for each document.
...
* `__HSEARCH_tenantId`: the "discriminator" field holding the tenant ID.
* `__HSEARCH_id`: a field holding the the original (tenant-scoped) document ID.
