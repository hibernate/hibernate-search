...
This operation does not always improve performance: see <<mapper-orm-indexing-force-merge>>.
`forceMergeAsync()`::
Asynchronous version of `forceMerge()` returning a `CompletableFuture`.
This operation does not always improve performance: see <<mapper-orm-indexing-force-merge>>.

[NOTE]
[[mapper-orm-indexing-force-merge]]
.Force-merge and performance
====
The force-merge operation may affect performance positively as well as negatively.

Force-merge will regroup all index data into a single, huge segment.
...
There are, however, two situations in which force-merge may help:
