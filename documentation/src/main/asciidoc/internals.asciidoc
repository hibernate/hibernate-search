...
The idea is that the `SearchMappingRepositoryBuilder` will allow one or more initiators
...
This is done through the `ChangesetIndexWorker` class.
The mapper should create a worker whenever it needs to execute a series of works.

`ChangesetIndexWorker` carries *some* context usually associated to a "session" in the JPA world,
...
NOTE: For now index-scoped operations such as flush, optimize, etc. are unavailable from workers.
