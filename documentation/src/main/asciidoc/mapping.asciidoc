...
Note that this values is converted with a precision to the millisecond.
+
Note that it must be possible to convert the `Instant` or the `Duration` to a `Long`.
If this values are too big or too small an exception is thrown.
+

`LocalDate`, `LocalTime`, `LocalDateTime`, `LocalTime`, `MonthDay`,`OffsetDateTime`, `OffsetTime`,
`Period`, `YearMonth`, `ZoneDateTime`, `ZoneId`, `ZoneOffset`:: the bridges for this classes in
the `java.time` package store the values as string padded with 0 when required to allow sorting. 
