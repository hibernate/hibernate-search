...
via `@SortField#forField()`. Several sortable fields can be defined with help of the `@SortableFields` annotation.

The field to be marked as sortable must not be analyzed.

Note that sorting also works if a property is not explicitly marked with `@SortField`. This has negative runtime
...
Fields added through class-level bridges or custom field bridges cannot be marked as sortable by means of the
`@SortableField` annotation. If you need to sort on such fields, you therefore should add the required doc value fields
yourself from within the bridge implementation, in addition to the document fields it adds:

[[example-adding-docvaluefield]]
.Adding a doc value field
====
[source, JAVA]
----
public class MyClassBridge implements FieldBridge {
