...
* [[field-annotation-norms]]`norms`: describes whether index time boosting information should be stored (`Norms.YES`) or
...
but an easier way may be to use the `hibernate.search.lucene.analyzer_definition_provider`
configuration property.

This property can be set to the fully-qualified name of a class with a public, no-arg constructor in your application.
This class must either implement `org.hibernate.search.analyzer.definition.LuceneAnalyzerDefinitionProvider` directly
...
.Analyzer definition using a LuceneAnalyzerDefinitionProvider
====
[source, JAVA]
----
