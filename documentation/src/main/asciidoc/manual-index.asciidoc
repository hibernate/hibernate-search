...
Hibernate Search provides a new mass indexing approach using batch job introduced by JSR352.
It covers not only the existing functionality of the previous indexer, but also benefits the
powerful standard based facilities of the Java Batch Platform (JSR-352), such as checkpoint and
step partitioning. However, it requires a batch runtime for the execution:

* For Java EE users, we use the batch runtime provided by your Java EE container.
* For Java SE users, we use the batch runtime
  https://github.com/jberet/jsr352/tree/master/jberet-se[JBeret SE].

.How to use the new mass-indexer?
====
[source, JAVA]
----
...
In the following paragraphs, we will explain why JSR-352 standard is a good choice, and how to
configure different parameters in order to bootstrap the mass-indexing job.

==== Define your indexing scope
Our job allows you to define your own indexing scope—you can start a full indexation or a partial
indexation through 3 different methods: selecting the desired entity types, using HQL, or using
Hibernate criteria.

.How to add define the indexing scope in different methods?
====
[source, JAVA]
----
// full indexation
...
==== Parallelism
In order to maximize the performance, we highly recommend you to speed up the mass indexer using
parallelism. Parallelism is activated by default. Under the standard JSR 352, the exact word is
_"partitioning"_: The indexing step may run as multiple partitions, one per thread. Each partition
has its own partition ID and parameters. If there’re more partitions than threads, partitions are\
considered as a queue to consume: each thread can only run one partition at a time and won’t consume
the next partition until the previous one is finished.
...
available for different partitions. The default number of threads used in Hibernate Search is 10,
which correspond to the max number of threads in JBeret. You can overwrite you maximum number used
by Hibernate Search batch job, but we cannot guarantee that your batch runtime can provide the
requested number.
...
use as it can up to the requested maximum. (JSR352 v1.0 Final Release, page 34)
====

This approach is optimized for best performance but requires to set the application in maintenance
mode. Running a mass indexer in busy hours is not recommended. You also need to ensure the batch
runtime has enough available threads for the execution, because all the batch jobs share the same
thread pool.

===== Row per partitions
Mass indexer defines dynamically the number of partitions, but you need to tell it the number of
rows per partitions so that it can calculate the partitions by division. Each partition deals with
one root entity type, so 2 different entity types will never run under the same partition.
...
==== Checkpoint
Mass indexer supports checkpoint algorithm, which helps you to restart your suspended or failed job
from the last successful checkpoint. Assume that N is the value of checkpoint interval, then a
partition will reach the checkpoint at every N items processed. You can overwrite the default
...
Checkpoint a partition-scope algorithm, which means each partition has its own checkpoint counter
...
If you want to use multiple persistence unit, you will have to add two parameters when launching the
mass indexer:
...
If you want to use multiple persistence unit, you will have to register those as beans in the CDI
context. Note that entity manager factories may not be considered as beans by default, in which case
