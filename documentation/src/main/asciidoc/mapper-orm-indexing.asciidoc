...
The property `hibernate.search.automatic_indexing.strategy` controls how entities are indexed:

* by default, or when set to `session`, each change to an indexed entity
...
controls how Hibernate Search decides to reindex a updated entity:
