...
In order to do so, you will have to build a Lucene `Sort` object, which can be done by using the Hibernate Search sort DSL or the native Lucene `SortField` class.
...
Alternatively, you may build your sort using the `SortField` class directly. Then you will have to always specify the sort field type manually.
...
Sort sort = builder
  .sort()
    .byField("authorId", SortField.Type.LONG) // Sort type provided HERE
    .andByField("title")
  .createSort();
query.setSort(sort);
List results = query.list();
...
You may achieve similar mixes with Elasticsearch sorts when using the Elasticsearch backend. See <<elasticsearch-query-sorting>> for more information.
...
property is mapped to multiple fields using `@Fields` (see also <<fields-annotation>>). In this case
`forField` can be used to identify the index field to which it applies. Mirroring `@Fields` there
also exists a `@Facets` annotation in case multiple fields need to be targeted by faceting.
...
[[query-logging]]
==== Logging executed Lucene queries

Knowing the executed queries is vital when working on performance optimizations. This is especially
