...
Simple query string queries use the Lucene SimpleQueryParser. The query text is parsed and it exposes more
search features to the end user such as boolean operators and phrase queries.
...
Query luceneQuery = mythQB
    .simpleQueryString()
    .onField("history")
    .matching("(storm OR hurricane) AND \"falling branches\"")
    .createQuery();
----
====

By default, a simple query string query uses the OR operator if the operator is not explicitly defined.
If you prefer using the AND operator as default, you can add the `useAndAsDefaultOperator` flag.
...
of keywords in e.g. "name", "history" and "description", potentially with different boosts.
...
The syntax supported by the SimpleQueryParser includes the following operators: boolean (AND, OR, NOT),
prefix, phrase, precedence, fuzzy and near operator for phrase queries.
