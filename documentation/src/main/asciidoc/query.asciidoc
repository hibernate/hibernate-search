...
If you need more-like-this predicates in Hibernate Search 6,
feel free to drop a comment to explain your use case on
https://hibernate.atlassian.net/browse/HSEARCH-3272[HSEARCH-3272].
====
...
[WARNING]
====
Support for selecting facets ("drill-down") will be removed in Hibernate Search 6.

In order to drill-down and have your query return only specific facets,
wrap it in a <<_combining_queries,boolean query>> and
add a filter clause (`filteredBy`)
to constraint values of the facet field with either a <<_keyword_queries,keyword query>>
or a <<_range_queries,range query>>.

See also https://docs.jboss.org/hibernate/search/6.0/migration/html_single/#searching-facets-selectfacets[this section of the migration guide].
====
