 ...
 * parameters expecting a list of file paths in Lucene only accept *one* file path in Elasticsearch
 * file path parameters must refer to files on the Elasticsearch servers, be it absolute paths
   or paths relative to the Elasticsearch configuration directory.
 ...
 * some built-in Lucene factories are not (and cannot) be translated, because of incompatible
   ...
   This is in particular the case for `HunspellStemFilterFactory` and `TypeTokenFilterFactory`.
