...
Custom class used to configure the HTTP Client:: `hibernate.search.default.elasticsearch.http_client_configurer com.example.MyElasticsearchHttpClientConfigurer` (default is empty)
+
Allows fine tuning of the underlying Apache HTTP Client.
This enables in particular connecting to cloud services requiring particular authentication method,
...
This class must either implement `org.hibernate.search.elasticsearch.client.ElasticsearchHttpClientConfigurer` directly
...
on startup.
See http://hc.apache.org/httpcomponents-client-ga/ for more information on Apache HTTP Client
