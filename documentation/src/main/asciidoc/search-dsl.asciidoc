...
beyond the time limit, a `org.hibernate.search.util.common.SearchException` will be raised.
...
----
<1> Set `failAfter` timeout on terminal query DSL
<2> Fetch the results
<3> Optionally extract _took_: how much time the query took
<4> Optionally extract _timedOut_: whether the query timed out
====
...
----
<1> Set `truncateAfter` timeout on terminal query DSL
<2> Fetch the results
<3> Optionally extract _took_: how much time the query took
<4> Optionally extract _timedOut_: whether the query timed out
====
...
[WARNING]
====
This approach at the moment is not compatible with the `failAfter` approach.
If you try to set both `failAfter` and `truncateAfter`, the last set will override the previous one.
====
[NOTE]
====
`explain()` and `fetchTotalHitCount()` do not honor the timeout.
The former is used for debug purposes and in particular to find out why a query is slow.
For the latter it doesn't have any sense returning a *partial* result.
====
