...
==== Elasticsearch: `took` and `explanation`

.Returning query took time and if the time out has occurred
====
[source, JAVA, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/query/QueryDslIT.java[tags=explain-took-timedOut]
----
<1> Extract the `ElasticsearchSearchQuery` using the `ElasticsearchExtension`.
<2> Invoke `fetchAll()` to get a `ElasticsearchSearchResult`.
<3> Extract *took*.
<4> Extract *timedOut*.
====
