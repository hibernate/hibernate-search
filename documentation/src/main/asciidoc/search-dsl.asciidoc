...
==== Elasticsearch: `took` and `timed_out`

.Returning query execution time and whether a timeout occurred
====
[source, JAVA, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/query/QueryDslIT.java[tags=elasticsearch-took-timedOut]
----
<1> Extract the `ElasticsearchSearchQuery` using the `ElasticsearchExtension`.
<2> Fetch the results.
<3> Extract _took_: how much time the query took on the Elasticsearch side
(ignoring network latency between the application and the Elasticsearch cluster).
<4> Extract _timedOut_: whether the query timed out on the Elasticsearch side
(ignoring network latency between the application and the Elasticsearch cluster).
====
