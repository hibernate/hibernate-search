...
for example they are both `Integer`, or they are both `LocalDate` with the same date format, etc.
2. The fields must have a compatible DSL converter (for predicates and sorts) or projection converter (for projections).
2. For full-text predicates, the fields must have a compatible analyzer.
...
Your only option is to change your mapping,
renaming some fields or changing their type to avoid the conflict.

If you need to apply a similar predicate to two fields with different names and incompatible codec,
you will have to use two separate predicates and combine them with a <<search-dsl-predicate-boolean,boolean junction>>.
...
=== Incompatible decimal scale

Incompatible decimal scales are only a problem when the query is performed on scaled number fields among different indexes.

Such queries requires that the fields involved not only have the same encoded type (for instance both as `BigDecimal`),
but they must have the same decimal scale.

For scaled number fields and decimal scale attribute see <<mapper-orm-directfieldmapping-annotations-scalednumberfield, here>>.