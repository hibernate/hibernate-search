...
[CAUTION]
====
Failure handlers should *never* throw *any* exception.
Doing so will result in undetermined behavior,
so make sure to catch any exception occurring in failure handlers.
====
...
<5> Make sure to catch any exception occurring in the failure handler.
<6> `handle(IndexFailureContext)` is called for failures that are related to one index in particular.
<7> On top of the failing operation and throwable,
...
<8> Use the context-provided information to report the failure in any relevant way.
<9> Make sure to catch any exception occurring in the failure handler.
