...
[WARNING]
====
This specific version being an early preview and requiring Hibernate ORM version 5,
does not yet provide updated modules to run on WildFly.
WildFly 10 will soon be updated to include these out of the box.

We will again publish updated modules as usual in the near future.
====

==== More about modules

More information about the modules configuration in WildFly can be found in the
link:https://docs.jboss.org/author/display/WFLY9/Class+Loading+in+WildFly[Class Loading in WildFly 9] wiki.
