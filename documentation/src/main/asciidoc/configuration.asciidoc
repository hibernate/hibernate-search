...
and examples see link:http://infinispan.org/docs/7.2.x/user_guide/user_guide.html#_use_one_of_the_pre_configured_jgroups_files[usage of pre-configured JGroups stacks]
...
Hibernate Search is included in the WildFly application server, but you need to activate the module
...
unzip a different version provided by Hibernate Search.
The modules system in WildFly allows to safely run multiple versions of Hibernate ORM and
Hibernate Search in parallel, but make sure the Hibernate Search version you choose is compatible
with the Hibernate ORM version you choose.
...
[WARNING]
====
This version of Hibernate Search `{hibernateSearchVersion}` requires Hibernate ORM 5.
At the time of writing this paragraph the only version of WildFly compatible with Hibernate ORM 5 is WildFly version `10.0.0.Alpha5`;
since this is a preview version of the application server further changes might be needed.

If you need an Hibernate Search version for WildFly versions 9 or earlier, you can either include
...
Hibernate Search version included in each WildFly release (see <<using-wildfly-provided-hibernatesearch-versions>>).
====
