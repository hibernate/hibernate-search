...
See also the link:http://docs.hibernate.org/orm/{hibernateShortVersion}/userguide/html_single/Hibernate_User_Guide.html#jndi/[JNDI configuration in Hibernate ORM].
...
[WARNING]
====
Support for non-exclusive index use (setting this property to `false`) is deprecated,
...
WildFly 17 includes an older version of Hibernate Search; if you want to use that then you won't need to download anything else and you can stop reading this chapter; however you might miss out on several new features and improvements.
