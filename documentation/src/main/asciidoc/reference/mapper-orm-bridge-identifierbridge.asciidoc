...
You can define string parameters to the `@IdentifierBinderRef` annotation and then use them later in the binder:

.Passing parameters to an `IdentifierBridge` using the `@TypeBinderRef` annotation
====
[source, JAVA, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/mapper/orm/bridge/identifierbridge/parameter/context/Book.java[tags=include;!getters-setters]
----
...
[source, JAVA, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/mapper/orm/bridge/identifierbridge/parameter/context/OffsetIdentifierBinder.java[tags=include]
----
