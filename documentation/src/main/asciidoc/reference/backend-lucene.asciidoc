...
`default`::
The analyzer used by default with <<mapper-orm-directfieldmapping-annotations-fulltextfield,`@FullTextField`>>.
+
...
== Retrieving low-level Lucene `IndexReader`
...
to filter the visibility of the returning index reader using the method `openIndexReader(Set<String>)`.
====
...
[IMPORTANT]
====
The returned IndexReader instance must be closed after its use by the client.
====
