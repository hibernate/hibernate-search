...
+
For a field that is tokenized on spaces (using an analyzer), the regexp `pass:[.*love .* robots?]` will never match anything, because it requires a space inside the token and indexed tokens don't contain any (since tokenization happens on spaces).
+
...
* the regexp `pass:[Robots?]` will **not** be normalized and will never match anything, because it requires an uppercase letter and indexed tokens don't contain any (since they are lowercased).
...
+
For example `pass:[a~bc]` matches `aac` but not `abc`.
+
Don't forget to use parentheses if you need to negate a composite pattern, e.g. `pass:[a~(bc)]`
* `INTERVAL`: the `<>` operator matches a non-negative integer range, both ends included.
+
For example `pass:[a<1-10>]` matches `a1`, `a2`, ... `a9`, `a10`, but not `a11`.
+
...
+
For example `pass:[.*a.*&.*z.*]` matches `az`, `za`, `babzb`, `bzbab`, but not `a` nor `z`.
* `ANYSTRING`: the `@` operator matches any string; equivalent to `pass:[.*]`.
+
This operator is mostly useful to negate a pattern, e.g. `pass:[@&~(ab)]` matches anything except the string `ab`.
...
With `matchingAll` we require that all of the provided terms match.
...
.Matching all of the provided terms
====
[source, JAVA, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/predicate/PredicateDslIT.java[tags=terms-any]
