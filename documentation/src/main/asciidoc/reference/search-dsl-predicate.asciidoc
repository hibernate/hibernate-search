...
[[search-dsl-predicate-boolean-clauses-must]]`must` / `and`::

`must` / `and` clauses are required to match: if they don't match, then the `bool` predicate will not match.
+
Matching "must" / "and" clauses are taken into account during <<search-dsl-predicate-common-score,score>> computation.
+
NOTE: `must` and `and` clauses are equivalent and interchangeable. The `must` naming directly comes from
Lucene while `and` was added as syntactic sugar for users who find such naming easier to grasp.
...
[[search-dsl-predicate-boolean-clauses-should]]`should` / `or`::
`should` / `or` clauses may optionally match, and are required to match depending on the context.
+
Matching `should` / `or` clauses are taken into account during <<search-dsl-predicate-common-score,score>> computation.
+
The exact behavior of `should` / `or` clauses is as follows:
+
* When there isn't any `must` / `and` clause nor any `filter` clause in the `bool` predicate
...
* When there is at least one `must` / `and` clause or one `filter` clause in the `bool` predicate,
...
A `bool` predicate with only `should` / `or` clauses will behave as an `OR` operator.
...
A `bool` predicate with only `must` / `and` clauses will behave as an `AND` operator.
...
`filter` clauses are essentially `must` / `and` clauses with only one difference:
...
=== `should` / `or` as a way to tune scoring

Apart from being <<search-dsl-predicate-boolean-or,used alone to emulate an `OR` operator>>,
`should` / `or` clauses can also be used in conjunction with `must` / `and` clauses.
When doing so, the `should` / `or` clauses become completely optional,
...
=== `minimumShouldMatch`: fine-tuning how many `should` / `or` clauses are required to match

It is possible to require that an arbitrary number of `should` / `or` clauses match
...
To match every document when there is no clause, add a `must` / `and` clause that matches everything.
