...
[WARNING]
====
With `matchingAll` the given terms must not be more than 1024.
====
...
If it is true, this means that in order to match the related documents,
you need to provide the terms as they were written to the indexes.

This is always true with the Lucene backend.
For the Elasticsearch backend: terms are never analyzed nor normalized if an analyzer is defined on the field.
While terms are normalized if a normalizer is defined on the field.
