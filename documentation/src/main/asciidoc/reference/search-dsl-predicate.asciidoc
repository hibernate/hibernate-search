...
[IMPORTANT]
====
The provided regular expressions used in regexp predicates
may be normalized or not, it depends on the backend, if a normalizer has been defined on the field.
The same will never be tokenized
even if a analyzer with a proper tokenizer has been defined on the field.

This means that if a normalizer/analyzer has been defined on the field,
the user sometimes needs to be aware about how this normalizer/analyzer processes the data
in order to provide a meaningful regular expression for using the predicate.
====
