...
will never be normalized or transformed in any way,
even if a normalizer/analyzer has been defined on the field.

This means that if a normalizer/analyzer has been defined on the field,
the user needs to be aware about how this normalizer/analyzer processes the data
