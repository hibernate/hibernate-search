...
That is equivalent to having an <<search-dsl-predicate-boolean-or>> applied to the series of single-term predicates.
...
That is equivalent to having an <<search-dsl-predicate-boolean-and>> applied to the series of single-term predicates.
...
Differently from other predicates, the provided terms arguments to `matchAny` or `matchAll`
are never analyzed nor normalized.
This means that in order to match the related documents,
we need to provide the terms as they were written to the indexes.
...
== `simpleQueryString`: match a user-provided query string
// Search 5 anchors backward compatibility
...
Hibernate Search automatically wraps a nested predicates around other predicates when necessary.
