...
You can define string parameters to the `@TypeBinderRef` annotation and then use them later in the binder:

.Passing parameters to a `ValueBridge` using the `@TypeBinderRef` annotation
====
[source, JAVA, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/mapper/orm/bridge/valuebridge/parameter/context/Book.java[tags=include;!getters-setters]
----
...
[source, JAVA, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/mapper/orm/bridge/valuebridge/parameter/context/BooleanAsStringBinder.java[tags=include]
----
