...
[NOTE]
====
This optimization has no effect on calls to `fetchHits(...)` (which is already optimized by default) or to `fetchTotalHitCount()` (which always returns an exact hit count).
====
...
[source, JAVA, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/query/QueryDslIT.java[tags=fetching-totalHitsThreshold]
----
<1> Define a `totalHitsThreshold` for the current query
...
<<mapper-orm-bridge-routingbridge-routingkey,it is assigned a routing key>> at indexing time.
====
