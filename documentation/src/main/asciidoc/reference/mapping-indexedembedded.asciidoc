...
in <<bridge-index-field-dsl,some APIs exposed to custom bridges>>:
...
====
<<bridge-index-field-dsl-dynamic,Dynamic fields>> are not directly affected by filtering rules:
...
and configuring reindexing with <<mapper-orm-reindexing-associationinverseside,`@AssociationInverseSide`>>
