...
# Node #2
hibernate.search.coordination.strategy = outbox-polling
hibernate.search.coordination.agents.event_processor.enabled = false
----
...
[source]
----
hibernate.search.coordination.agents.event_processor.enabled = true
hibernate.search.coordination.agents.event_processor.polling_interval = 100
hibernate.search.coordination.agents.event_processor.pulse_interval = 2000
hibernate.search.coordination.agents.event_processor.pulse_expiration = 30000
hibernate.search.coordination.agents.event_processor.batch_size = 50
hibernate.search.coordination.agents.event_processor.transaction_timeout = 10
hibernate.search.coordination.agents.event_processor.retry_delay = 15
----

* `agents.event_processor.enabled` defines whether the event processor is enabled,
...
* `agents.event_processor.polling_interval` defines how long to wait for another query to the outbox events table
...
* `agents.event_processor.pulse_interval` defines how long the event processor can poll for events
...
* `agents.event_processor.pulse_expiration` defines how long an event processor "pulse" remains valid
...
* `agents.event_processor.batch_size` defines how many outbox events, at most, are processed in a single transaction
as an <<configuration-property-types,integer value>>.
...
* `agents.event_processor.transaction_timeout` defines the timeout for transactions processing outbox events
...
* `agents.event_processor.retry_delay` defines the time after which it is possible to process again an event that has gone into error.
