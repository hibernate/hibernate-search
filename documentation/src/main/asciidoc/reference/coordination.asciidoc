...
in order to <<coordination-database-polling-sharding-dynamic,dynamically assign shards>> to each application instance,
or simply to check that <<coordination-database-polling-sharding-static,statically assigned shards>>
are consistent.
...
Hibernate Search <<coordination-database-polling-schema,adds an agent table to the database>>,
and uses that table to register agents involved in indexing (most of the time, one application instance = one agent =
the <<coordination-database-polling-processors,indexing processor>>).
...
if using <<coordination-database-polling-sharding-static,static sharding>>,
e.g. two agents assigned to the same shard;
* trigger rebalancing as necessary if using <<coordination-database-polling-sharding-dynamic,dynamic sharding>>;
...
but you may want to configure <<coordination-database-polling-processors, the background event processor>>.
...
+
See <<coordination-database-polling-sharding-basics,the sharding basics>> for information about "pulses".
+
