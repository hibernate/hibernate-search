...
+
A <<coordination-outbox-polling-processors,background processor>> polls the database for new entity change events,
...
+
You can <<coordination-outbox-polling-processors,configure this background processor>> to run more often,
...
* Each application node involved in <<coordination-outbox-polling-processors,event processing>>
...
the <<coordination-outbox-polling-processors,indexing processor>>).
...
or that agents left the cluster voluntarily,
or that agents expired (did not update their entry for a long time).
...
but you may want to configure <<coordination-outbox-polling-processors, the background event processor>>.
...
when <<coordination-outbox-polling-processors,processing is disabled>>.
====
...
to disable <<coordination-outbox-polling-processors,processing>>,
...
[[coordination-outbox-polling-processors]]
=== [[coordination-database-polling-processors]] Processors

The `outbox-polling` coordination strategy involves background processors,
in particular one that polls the outbox table for events
and then reindex the corresponding entities when new events are found.

The background indexing processor can be configured using the following configuration properties:
...
hibernate.search.coordination.processors.indexing.pulse_interval = 2000
hibernate.search.coordination.processors.indexing.pulse_expiration = 30000
...
hibernate.search.coordination.processors.indexing.retry_delay = 15
----
...
* `processors.indexing.pulse_interval` defines how long the background indexing processor can poll for events
...
* `processors.indexing.pulse_expiration` defines how long a background indexing processor "pulse" remains valid
...
but increased risk of incorrectly considering an agent as expired.
+
High values (much larger than the pulse interval) mean a longer delay before rebalancing
when a node abruptly leaves the cluster due to a crash or network failure,
but reduced risk of incorrectly considering an agent as expired.
...
* `processors.indexing.retry_delay` defines the time after which it is possible to process again an event that has gone into error.
