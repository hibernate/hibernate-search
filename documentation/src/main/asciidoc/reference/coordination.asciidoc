...
hibernate.search.coordination.entity.mapping.agent.uuid_type=uuid-binary
...
hibernate.search.coordination.entity.mapping.outboxevent.uuid_type=uuid-binary
----
...
+
Defaults to `auto`.
* `agent.uuid_type` defines link:{hibernateDocUrl}#basic-custom-type[the name of the registered Hibernate `BasicType`]
used for representing an UUID in the agent table. Hibernate Search provides a special `default` option that is going to
be used by default and will result into one of `uuid`/`uuid-binary`/`uuid-char` depending on the database in use.
Please refer to link:{hibernateDocUrl}#basic-provided[Hibernate-provided BasicTypes] to see
the list of available UUID representations provided by Hibernate ORM. A user defined type can also be supplied.
+
Defaults to `default`.
...
+
Defaults to `auto`.
* `outboxevent.uuid_type` defines link:{hibernateDocUrl}#basic-custom-type[the name of the registered Hibernate `BasicType`]
...
+
* When using time-based UUIDs for outbox events (see <<coordination-database-polling-schema>>), picks `id`.
...
+
If outbox event identifiers are time-based UUIDs (see <<coordination-database-polling-schema>>),
this behaves similarly to `time`, but without the risk of deadlocks.
+
If outbox event identifiers are random UUIDs (see <<coordination-database-polling-schema>>),
