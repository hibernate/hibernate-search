...
----
include::{sourcedir}/org/hibernate/search/documentation/search/highlighting/BookWithHighlights.java[tags=basics]
----
...
<2> A full-text field that explicitly allows any highlighter types to be applied to it.
See the definition of the <<mapping-directfieldmapping-highlightable-any, `ANY` highlightable>> for any details.
<3> A full-text field that implicitly allows any highlighter types to be applied to it.
...
<4> A generic text field -- such field will not allow highlight projections.
<5> A full-text field that explicitly allows for plain and unified highlighters to be applied to it.
====
...
.Specifying the highlighter type
====
[source, JAVA, indent=0, subs="+callouts"]
----
...
<2> Starting the definition of a unified highlighter.
<3> Starting the definition of a fast vector highlighter.
====
...
but only by plain and fast-vector highlighters with the Lucene backend.
====
...
== Maximum analyzed offset

The maximum analyzed offset option allows specifying the limit of characters to be analyzed by the highlighter.
Can be used to truncate long texts and only perform the highlighting on an initial text portion.

.Setting the maximum analyzed offset
====
[source, JAVA, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/highlighting/HighlightingDslIT.java[tags=phrase-limit]
----
<1> Configuring the maximum analyzed offset.
====

[NOTE]
====
This option is supported only by plain and unified highlighter types with the <<backend-elasticsearch, Elasticsearch backend>>
and only by the plain highlighter with the <<backend-lucene,Lucene backend>>.
====
