...
----
<1> Fields declared in object fields (created by <<mapper-orm-indexedembedded,`@IndexedEmbedded`>>, for example)
...
The only exception is <<mapper-orm-bridge-named-predicate,named predicates>> registered on object fields:
