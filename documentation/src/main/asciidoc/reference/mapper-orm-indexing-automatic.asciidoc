...
2+|<<mapper-orm-indexing-automatic-concepts-changes-in-session-jpql-sql,No>>
...
[[mapper-orm-indexing-automatic-concepts-changes-in-session-basics]]
=== Basics
...
[[mapper-orm-indexing-automatic-concepts-changes-in-session-jpql-sql]]
=== Limitation: no detection of changes caused by JPQL or SQL queries

Changes resulting from `insert`/`delete`/`update` queries, be it SQL or JPQL/HQL queries,
...
=== Limitation: asymmetric association updates
...
With coordination strategies such as <<coordination-database-polling>>,
...
|Best|`async`|No guarantee|No guarantee|No guarantee
|Medium|`write-sync` (**default**)|Guaranteed|Guaranteed|No guarantee
|Medium to <<mapper-orm-indexing-automatic-synchronization-refresh-throughput,worst>>|`read-sync`|Guaranteed|No guarantee|Guaranteed
|<<mapper-orm-indexing-automatic-synchronization-refresh-throughput,Worst>>|`sync`|Guaranteed|Guaranteed|Guaranteed
|====
