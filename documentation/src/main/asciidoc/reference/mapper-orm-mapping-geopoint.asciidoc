[[mapper-orm-geopoint]]
= [[spatial]] Mapping geo-point types

[[mapper-orm-geopoint-basics]]
== Basics
...
== [[spatial-coordinatesinterface]] Using `@GenericField` and the `GeoPoint` interface
...
== [[spatial-indexing-range]] [[spatial-indexing-spatialHash]] Using `@GeoPointBinding`, `@Latitude` and `@Longitude`
