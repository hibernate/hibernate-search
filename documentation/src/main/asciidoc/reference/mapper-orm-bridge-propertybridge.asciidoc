...
----
<1> The bridge must implement the `PropertyBridge<P>` interface.
...
The generic type `<P>` here reflects the type of the property on which we're going to apply the bridge,
