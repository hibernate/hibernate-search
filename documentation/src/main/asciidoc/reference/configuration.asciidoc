...
IndexSettings::
+
`IndexSettings.indexDefaultsKey(ElasticsearchIndexSettings.INDEXING_QUEUE_SIZE)`
is equivalent to `hibernate.search.backend.index_defaults.indexing.queue_size`.
+
`IndexSettings.indexDefaultsKey("myBackend", ElasticsearchIndexSettings.INDEXING_QUEUE_SIZE)`
is equivalent to `hibernate.search.backends.myBackend.index_defaults.indexing.queue_size`.
+
For a list of available property keys,
...
[TIP]
====
You can also use `IndexSettings.indexKey("myIndex", ElasticsearchIndexSettings.INDEXING_QUEUE_SIZE)`
or `IndexSettings.indexKey("myBackend", "myIndex", ElasticsearchIndexSettings.INDEXING_QUEUE_SIZE)`
