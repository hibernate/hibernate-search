...
= [[search-mapping-associated]] Mapping associated elements with `@IndexedEmbedded`
...
== [[_indexing_null_embeddeds]] `@IndexedEmbedded` and `null` values
...
== [[_associated_objects_building_a_dependency_graph_with_containedin]] Reindexing when embedded elements change
...
== [[search-mapping-associated-viapaths]] Filtering embedded fields and breaking `@IndexedEmbedded` cycles
...
== [[mapper-orm-indexedembedded-storage]] Structuring embedded elements as nested documents using `structure`
