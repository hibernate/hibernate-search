...
Optionally it is possible to provide from outside the full index settings to use when Hibernate Search creates indexes.
...
The provided settings will be used in place of the one Hibernate Search would have generated by default.

[WARNING]
====
When this option is active, all the usually automatically-generated index settings won't be generated anymore,
including the analysis definitions.
This means that all the required analyzers and normalyzers will have to be contained in the provided settings.
====
