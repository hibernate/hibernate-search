...
Hibernate Search can <<mapper-orm-schema-management,create and validate indexes>>, but by default created indexes will only include the bare minimum required to index and search: the mapping, and the analysis settings.
Should you need to set some link:{elasticsearchDocUrl}/mapping.html[custom index mappings], it is possible to provide these mappings to Hibernate Search: it will include them when creating an index.
If the index already exists, these mappings will be passed to the Elasticsearch server, which will validate the changes.
Thus, the user provided mappings will not be validated by Hibernate Search.
...
[IMPORTANT]
====
When this property is used Hibernate Search will try to merge the mappings created by the framework
for the mapped fields with the mappings provided by the user.
In case of conflicts, which happens when some properties overlap, the mappings provided will take precedence.
Which means that it is possible to break the mappings for the purposes of Hibernate Search functions.
====
