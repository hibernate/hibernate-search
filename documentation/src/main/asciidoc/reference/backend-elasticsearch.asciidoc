...
==== Provide complete URIs

Alternatively to `hosts` and `protocol`, it is possible to define both the terms using the same property:
...
[WARNING]
====
Note that there atrre some constraints we need to respect in order to use this property:

* All the uris must have the same protocol.
* Cannot be used if `hosts` or `protocol` are set.
* It must contain at least a valid URI.
====
