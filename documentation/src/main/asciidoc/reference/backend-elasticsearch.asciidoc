 ...
 **The year is always set to 0**.
 |-
|<<mapper-orm-geopoint-basics,GeoPoint>>
 |`geo_point`
 |-
|====
...
if no <<mapper-orm-bridge-index-field-dsl-dynamic>> are defined.
...
The multi-tenancy is automatically enabled on the backend if it is enabled in the mapper,
e.g.: if a multi-tenancy strategy is defined on Hibernate ORM,
see link:{hibernateDocUrl}#multitenacy[Multitenancy].
...
Hibernate Search can <<mapper-orm-schema-management,create and validate indexes>>, but by default created indexes will only include the bare minimum required to index and search: the mapping, and the analysis settings.
Should you need to customize some link:{elasticsearchDocUrl}/mapping-params.html[mapping paramters], it is possible to provide a custom mapping to Hibernate Search: it will include it when creating an index.

[CAUTION]
====
The consistency of the custom Elasticsearch mapping with the Hibernate Search will not get checked in any way. You are responsible for making sure that any override in your mapping can work, e.g. that you're not changing the type of an index field from `text` to `integer`, or disabling `doc_values` on a field used for sorting.
...
----
# To configure the defaults for all indexes:
hibernate.search.backend.schema_management.mappings_file = custom/index-mappings.json
# To configure a specific index:
hibernate.search.backend.indexes.<index name>.schema_management.mappings_file = custom/index-mappings.json
----

.Possible content of `custom/index-mappings.json` file
====
[source, JSON, indent=0, subs="+callouts"]
----
include::{resourcesdir}/custom/index-mappings.json[]
----
====
...
.Possible resulting mapping after merging the content of `custom/index-mappings.json` with the Hibernate Search mapping
====
[source, JSON, indent=0, subs="+callouts"]
----
include::{resourcesdir}/custom/index-settings-merged.json[]
----
...
For instance you could pass a `custom/index-mappings.json` file like the following:

.Possible content of `custom/index-mappings.json` file to disable the `_source` field
====
[source, JSON, indent=0, subs="+callouts"]
----
{
