...
* `HibernateOrmIndexingPlanSynchronizationStrategy.async()` for the Hibernate ORM mapper, or `PojoStandaloneIndexingPlanSynchronizationStrategy.async()` for the Pojo Standalone mapper
* `HibernateOrmIndexingPlanSynchronizationStrategy.writeSync()` for the Hibernate ORM mapper, or `PojoStandaloneIndexingPlanSynchronizationStrategy.writeSync()` for the Pojo Standalone mapper
* `HibernateOrmIndexingPlanSynchronizationStrategy.readSync()` for the Hibernate ORM mapper, or `PojoStandaloneIndexingPlanSynchronizationStrategy.readSync()` for the Pojo Standalone mapper
* or `HibernateOrmIndexingPlanSynchronizationStrategy.sync()` for the Hibernate ORM mapper, or `PojoStandaloneIndexingPlanSynchronizationStrategy.sync()` for the Pojo Standalone mapper
