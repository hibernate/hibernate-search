...
...
+
Full-text fields also allow <<add-link-later,highlighting>>.
+
...
|`Highlightable.DEFAULT`| Use the backend-specific default. <<backend-lucene,Lucene's>> default value is `Highlightable.NO`,
<<backend-elasticsearch,Elasticsearch's>> default value is `[Highlightable.PLAIN, Highlightable.UNIFIED]`
|===============
+
[IMPORTANT]
====
This option is *only* available for `@FullTextField`.
====
...
[[mapper-orm-directfieldmapping-supported-types-date-time]][[mapping-directfieldmapping-supported-types-date-time,Possibly lower range/resolution]]
[NOTE]
