...
<1> Get the `SearchSession`.
<2> Create a `MassIndexer` targeting every indexed entity type.
<3> Reindex only the books having year less or equals to 2100.
<4> Reindex only the author having birthday less of a given local date.
<5> For this case the date is passed as query parameter.
...
At the moment there is no way to filter the entities that will be purged.
