...
To retrieve the `SearchMapping` with the Hibernate ORM integration,
...
To retrieve the `SearchSession` with the Hibernate ORM integration,
...
<5> For advanced use cases, it is possible to target (indexed entity) types by their name,
