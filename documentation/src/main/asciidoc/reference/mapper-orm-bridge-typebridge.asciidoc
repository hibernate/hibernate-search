...
Optionally you can provide the expected type for the bridged element,
using the method `<P> bridge(Class<P>, TypeBridge<P>)`,
to statically define the type and so avoid casting on the bridge.
...
----
<1> The bridge must implement the `TypeBridge<T>` interface.
The generic type `<T>` here reflects the type of the entity on which we're going to apply the bridge,
that we'll call the bridged element.
