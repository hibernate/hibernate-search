...
Optionally you can provide the expected type for the bridged property,
using the method `<P> bridge(Class<P>, PropertyBridge<P>)`,
...
and pass it to the bridge for later use.
Instead of using the `bridge` method,
if the type of the bridged property is a `java.util.Map`,
it is possible to bind the bridge using the `mapPropertyBridge` method,
to statically define the types of the key and the value.
