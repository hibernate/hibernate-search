...
<3> It is possible to set the number of threads to be used. For the complete option list see <<mapper-orm-indexing-massindexer>>.
...
----
<1> Get a Hibernate Search mapping, called `SearchMapping`, from the `EntityManagerFactory`.
<2> Create a "search scope", representing the indexed types that will be queried.
<3> Initiate a search query targeting the search scope and relying on the given entity manager for entity loading.
...
i.e. how text is supposed to be processed when indexing and searching.
This involves _analyzers_,
which are made up of three types of components, applied one after the other:

* (rarely) zero or more character filters, to clean up the input text:
...
but it is generally better to build your own
by picking the filters and tokenizer most suited to your specific needs.
...
and <<search-dsl-aggregation,aggregations>>.
