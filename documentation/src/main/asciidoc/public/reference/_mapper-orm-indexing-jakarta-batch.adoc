...
|Default value depends on <<jakarta-batch-parameters-drop-and-create-schema,`dropAndCreateSchemaOnStart(boolean)`>>.
...
|[[jakarta-batch-parameters-drop-and-create-schema]]`dropAndCreateSchemaOnStart` / `.dropAndCreateSchemaOnStart(boolean)`
|`False` in general, `True` on <<backend-elasticsearch-compatibility-amazon-opensearch-serverless,Amazon OpenSearch Serverless>>
