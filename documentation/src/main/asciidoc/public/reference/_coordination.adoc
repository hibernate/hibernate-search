...
See the following subsections for details about <<coordination-none,how the default works>>,
as well as how to use other strategies -- namely <<coordination-outbox-polling,`outbox-polling`>>.
...
The `none` strategy is the simplest and does not involve any additional infrastructure
...
but can also be selected explicitly with the following settings:
...
see link:{hibernateDocUrl}#settings-hibernate.default_batch_fetch_size[the `batch_fetch_size` property]
...
While currently Hibernate Search will use the dialect's default link:{hibernateDocUrl}#basic-uuid[representation of the UUID] in the database,
