...
3+|Listener-triggered indexing ignores: <<limitations-changes-in-session,JPQL/SQL queries>>, <<limitations-changes-asymmetric-association-updates,asymmetric association updates>>; <<limitations-stateless-session,`StatelessSession` indexing requires outbox-polling>>
