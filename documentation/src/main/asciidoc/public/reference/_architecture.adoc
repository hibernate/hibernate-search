...
3+|Listener-triggered indexing ignores: <<limitations-changes-in-session,JPQL/SQL queries>>, <<limitations-changes-asymmetric-association-updates,asymmetric association updates>>; has <<limitations-stateless-session,limitations with `StatelessSession`>>
