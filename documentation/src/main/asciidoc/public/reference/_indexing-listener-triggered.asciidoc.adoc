...
|Detects changes occurring in ORM sessions (`session.persist(...)`, `session.delete(...)`, `statelessSession.insert(...)`, `statelessSession.update(...)`, setters, ...)
...
entities inserted, updated, or deleted through a `StatelessSession` will be indexed transparently.

This works great for most applications, but you need to consider some limitations:

* <<limitations-stateless-session,Limitations with `StatelessSession`>>
