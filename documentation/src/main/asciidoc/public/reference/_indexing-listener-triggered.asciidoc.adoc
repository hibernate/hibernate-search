...
|Supports `StatelessSession` for indexing
|No (<<limitations-stateless-session,throws an exception>>)
|<<limitations-stateless-session,Yes>>
...
Listener-triggered indexing also works with Hibernate ORM's `StatelessSession`,
but only with the <<coordination-outbox-polling,outbox-polling coordination strategy>>.
With the default coordination strategy, indexing through a `StatelessSession` will result in an exception.
See <<limitations-stateless-session>> for details.

This works great for most applications, but you need to consider some limitations:

* <<limitations-stateless-session,`StatelessSession` requires outbox-polling for indexing>>
