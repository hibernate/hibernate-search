...
And an incubating one based on the Lucene {luceneNextVersion}, that requires JDK 21+:
...
[NOTE]
====
This `hibernate-search-backend-lucene-next` backend is tested against the same test suite as the `hibernate-search-backend-lucene`
...
Starting with Lucene 10, it is leveraging new Java APIs, particularly to work with memory, that are available starting with JDK 21.
