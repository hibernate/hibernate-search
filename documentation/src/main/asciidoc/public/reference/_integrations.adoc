...
And an incubating one based on the Lucene {lucene10Version}, that requires JDK 21+:
...
[NOTE]
====
This `hibernate-search-backend-lucene10` backend is tested against the same test suite as the `hibernate-search-backend-lucene`
