...
== Listener-triggered indexing with `StatelessSession`

[[limitations-stateless-session-description]]
=== Description

Hibernate Search's <<listener-triggered-indexing,listener-triggered indexing>> with a `StatelessSession`
requires the <<coordination-outbox-polling,outbox-polling coordination strategy>>.

With the default coordination strategy (<<coordination-none,no coordination>>),
attempting to insert, update, or delete an indexed or embedded entity through a `StatelessSession`
will result in an exception.
This is to prevent stale data and incorrect index contents (because of the `StatelessSession` specifics).

Even with <<coordination-outbox-polling,outbox-polling>>, there are some differences
compared to using a regular `Session`:
...
Search operations from a `StatelessSession` (using `Search.session(StatelessSession)`)
work regardless of the coordination strategy.

[[limitations-stateless-session-solution]]
=== Solutions and workarounds

To use `StatelessSession` for indexing, configure the
<<coordination-outbox-polling,outbox-polling coordination strategy>>.

If outbox-polling is not an option, use a regular `Session` for indexing operations,
and reserve the `StatelessSession` for search operations only.

Alternatively, you could index entities explicitly,
either using the <<indexing-massindexer,`MassIndexer`>>,
using the <<mapper-orm-indexing-jakarta-batch,Jakarta Batch mass indexing job>>,
