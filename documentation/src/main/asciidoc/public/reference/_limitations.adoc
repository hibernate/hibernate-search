...
(such as optimized dirty checking, etc.), use a regular `Session` instead.

If your use of `StatelessSession` is not fully covered by the listener-triggered indexing,
...
using the <<mapper-orm-indexing-jakarta-batch,Jakarta Batch mass indexing job>>,
or <<indexing-explicit,explicitly through a `Session` based `SearchIndexingPlan`>>.
