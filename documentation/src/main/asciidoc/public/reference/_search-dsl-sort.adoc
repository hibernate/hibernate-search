...
include::../components/_field-sort-important.adoc[]
...
[[search-dsl-sort-withparameters]]
== `withParameters`: create sorts using query parameters

include::../components/_incubating-warning.adoc[]

The `withParameters` sort allows building sorts using <<search-dsl-query-parameters,query parameters>>.

This type of sort requires a function that accepts query parameters and returns a sort.
That function will get called at query building time.

.Creating a sort with query parameters
====
[source, JAVA, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/sort/SortDslIT.java[tags=with-parameters]
----
