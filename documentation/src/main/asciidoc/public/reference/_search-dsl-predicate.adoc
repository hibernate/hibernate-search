...
[[search-dsl-predicate-match-minimumshouldmatch]]
...
.Fine-tuning matching requirements with `minimumShouldMatch`
====
[source, java, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/predicate/PredicateDslIT.java[tags=match-minimumShouldMatchNumber]
----
...
[[search-dsl-predicate-disjunctionMax]]
== `disjunctionMax`: match any clause, scoring on the best one

The `disjunctionMax` predicate matches documents that match **any** of its inner predicates,
called "disjuncts" -- exactly like <<search-dsl-predicate-or,`or`>>.
The difference is in the <<search-dsl-predicate-common-score,score>>:
where `or` *sums* the scores of every matching disjunct,
`disjunctionMax` takes the *highest* score among them.

This is what you want when the disjuncts are alternative expressions of the *same* user intent --
most commonly the same text matched against several fields.
With `or`, a document that happens to repeat the search terms in many fields
outranks a document that matches one field very well;
with `disjunctionMax`, the best-matching field decides.

.Matching any of multiple predicates, scoring on the best match
====
[source, java, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/predicate/PredicateDslIT.java[tags=disjunctionMax]
----
<1> The hits *should* have a `title` field matching the text `robot`.
<2> The hits *should* have a `description` field matching the text `robot`.
<3> All returned hits will match *at least one* of the disjuncts above,
and will be scored on whichever of them matched best, not on the sum of both.
====

[[search-dsl-predicate-disjunctionMax-tieBreaker]]
=== Letting the other disjuncts contribute with `tieBreaker(...)`

By default only the best-scoring disjunct contributes to the score of a document.
`tieBreaker(...)` sets the multiplier applied to the *other* matching disjuncts,
so that a document matching several of them can still rank slightly higher
than a document matching only one -- without the number of matches dominating the score.

.Giving the non-best disjuncts a fraction of their score
====
[source, java, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/predicate/PredicateDslIT.java[tags=disjunctionMax-tieBreaker]
----
<1> Each matching disjunct that is not the best-scoring one contributes 30% of its score.
====

Values between `0` and `1` are the useful range; anything outside it is for experts only.
It defaults to `0`, meaning only the best-scoring disjunct counts.
A value of `1` makes every matching disjunct contribute its full score,
which is the same scoring as <<search-dsl-predicate-or,`or`>>.

[[search-dsl-predicate-disjunctionMax-other]]
=== Options

* The score of a `disjunctionMax` predicate is variable by default,
but can be <<search-dsl-predicate-common-constantScore,made constant with `.constantScore()`>>.
* The score of a `disjunctionMax` predicate can be <<search-dsl-predicate-common-boost,boosted>>
with a call to `.boost(...)`.
* Disjuncts can be added dynamically with `with(...)`,
exactly as for <<search-dsl-predicate-or-lambda,`or`>>.
...
[[search-dsl-predicate-simple-query-string-minimumshouldmatch]]
=== `minimumShouldMatch`: fine-tuning how many `should` clauses are required to match

The resulting query parsed from the query string may result in a boolean query with should clauses.
It may be helpful to control how many `should` clauses must match to consider a document as a match.

.Fine-tuning `should` clauses matching requirements with `minimumShouldMatch`
====
[source, java, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/predicate/PredicateDslIT.java[tags=simpleQueryString-minimum-should-match]
----
====

This is similar to <<search-dsl-predicate-boolean-minimumshouldmatch,boolean>> or
<<search-dsl-predicate-query-string-minimumshouldmatch,query string>>
predicates `minimumShouldMatch` options.
...
[[search-dsl-predicate-simpleQueryString-format-types]]
=== Field types and expected format of field values

This predicate is applicable to most of the <<mapping-directfieldmapping-supported-types,supported field types>>
except `GeoPoint` and <<mapping-directfieldmapping-annotations-vectorfield,vector field>> types.

The format of string literals used in the query string is backend-specific.
With the Lucene backend, the format of these literals should be compatible with the parsing logic defined in <<mapping-directfieldmapping-supported-types-table>>,
and for fields with custom bridges it <<binding-valuebridge-parse,must be defined >>.
As for the Elasticsearch backend, see the <<backend-elasticsearch-field-types-table>>.

Keep in mind that *not* all query constructs can be applied to non-string fields,
e.g. adding <<search-dsl-predicate-simple-query-string-fuzzy,fuzziness>>,
<<search-dsl-predicate-query-string-phrase-slop,slop>> or <<search-dsl-predicate-simple-query-string-prefix,wildcards>>
will not work.
...
[[search-dsl-predicate-knn-with-similarity]]
=== Filtering out irrelevant results with `knn` similarity

By its nature a `knn` predicate will always try to find `k` nearest vectors,
...
If configured, `knn` predicate will find `k` nearest vectors
...
<<mapping-directfieldmapping-vectorSimilarity,vector similarity>>.

.Filtering out irrelevant results
====
[source, java, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/predicate/PredicateDslIT.java[tags=knn-similarity]
----
...
With the <<backend-lucene,Lucene backend>> `k` is the number that will limit the final amount of documents
matched by a <<search-dsl-predicate-knn,`knn` predicate>>.
When using the <<backend-elasticsearch-compatibility-elasticsearch,Elastic>> distribution of the <<backend-elasticsearch,Elasticsearch backend>>
`k` will be treated as both `k` and `num_candidates`.
...
While when an <<backend-elasticsearch-compatibility-opensearch,OpenSearch>> distribution is used,
...
== `queryString`: match a user-provided query string

The `queryString` predicate matches documents according to a structured query given as a string.
It allows building more advanced query strings and has more configuration options than a <<search-dsl-predicate-simple-query-string,`simpleQueryString`>> predicate.

We will not go much into details of a query syntax in this guide
To familiarize yourself with it, please refer to your backend
(link:{elasticsearchDocUrl}/query-dsl-query-string-query.html#query-string-syntax[Elasticsearch]/link:{openSearchDocUrl}/query-dsl/full-text/query-string/[OpenSearch]/link:{luceneQueryparserJavadocUrl}org/apache/lucene/queryparser/classic/package-summary.html#package.description/[Lucene])
guides.
...
[source, java, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/predicate/PredicateDslIT.java[tags=queryString-query]
----
...
[[search-dsl-predicate-query-string-boolean-operators-default]]
=== Default boolean operator
...
.Matching a query string: AND as default operator
====
[source, java, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/predicate/PredicateDslIT.java[tags=queryString-defaultOperator-and]
----
====

[[search-dsl-predicate-query-string-phrase-slop]]
=== Phrase slop

The phrase slop option defines how permissive a constructed phrase predicate will be;
in other words, how many transpositions in the phrase are allowed for it to still be considered a match.
With query predicate, this option can be set in the query string itself.

.Matching a query string: phrase slop as part of query string itself
====
[source, java, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/predicate/PredicateDslIT.java[tags=queryString-phrase-slop-through-query]
----
====

Alternatively, `.phraseSlop(...)` can be applied to a query string predicate.

.Matching a query string: phrase slop as predicate option
====
[source, java, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/predicate/PredicateDslIT.java[tags=queryString-phrase-slop-through-parameter]
----
====

Note that passing the value to `.phraseSlop(...)` sets the default phrase slop value, that can be overridden in the query string.

.Matching a query string: phrase slop as predicate option overridden by query
====
[source, java, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/predicate/PredicateDslIT.java[tags=queryString-phrase-slop-through-parameter-overridden]
----
...
[[search-dsl-predicate-query-string-allow-leading-wildcard]]
=== Allowing leading wildcards

A query string can use a wildcard at any position within a query by default.
If there is a need to prevent users from using leading wildcards, `.allowLeadingWildcard(..)` can be called
with a `false` value to disallow such queries.

.Matching a query string: disallowing leading wildcards
====
[source, java, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/predicate/PredicateDslIT.java[tags=queryString-allow-leading-wildcard]
----
In this example, a query uses the wildcard at the end of the word; hence, it is acceptable regardless of the option value.
If queries like `?obot` or `*bot` were supplied, and at the same time, leading wildcards were disallowed
...
For example, in a query string `robot ?etective` the wildcard `?` is not a leading character,
but this query is broken down as two clauses for `robot` and `?etecitve` where in the second
clause, the `?` wildcard becomes a leading character.

[[search-dsl-predicate-query-string-enable-position-increments]]
=== Enabling position increments

Position increments are enabled by default.
Position increments are enabled by default, allowing phrase queries to take into account stop words removed by a `stopwords` filter.
Position increments can be disabled as shown below,
leading to a change in the behaviour of phrase queries:
assuming that there is a phrase `book at the shelve` in the document, and the stop filter removes both `at` and `the`,
with disabled position increments, phrase query `"book shelve"` will not match such document.

.Matching a query string: disabling position increments
====
[source, java, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/predicate/PredicateDslIT.java[tags=queryString-enable-position-increments]
----
====

[[search-dsl-predicate-query-string-rewrite-method]]
=== Rewrite method

The rewrite method determines how the backend's query parser rewrites and scores multi-term queries.

To change the default `CONSTANT_SCORE` rewrite method, one of the allowed `RewriteMethod` enum values can be passed
to `.rewriteMethod(RewriteMethod)`/`rewriteMethod(RewriteMethod, int)`.
...
To achieve a constant score for results, see <<search-dsl-predicate-query-string-other,this documentation section>> on the query string predicate.

We will not go into details about different rewrite methods in this guide.
To learn more about them, please refer to your backend's guide
(link:{elasticsearchDocUrl}/query-dsl-multi-term-rewrite.html[Elasticsearch]/link:{openSearchDocUrl}/query-dsl/full-text/query-string/[OpenSearch]/link:{luceneJavadocUrl}org/apache/lucene/search/MultiTermQuery.html[Lucene]).

.Matching a query string: rewrite method
====
[source, java, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/predicate/PredicateDslIT.java[tags=queryString-rewrite-method]
----
====

[[search-dsl-predicate-query-string-minimumshouldmatch]]
=== `minimumShouldMatch`: fine-tuning how many `should` clauses are required to match
...
This is similar to <<search-dsl-predicate-boolean-minimumshouldmatch,boolean>> or
<<search-dsl-predicate-simple-query-string-minimumshouldmatch,simple query string>>
predicates `minimumShouldMatch` options.

[[search-dsl-predicate-query-string-multiple-fields]]
=== Targeting multiple fields
...
[[search-dsl-predicate-query-string-format-types]]
=== Field types and expected format of field values
...
Keep in mind that *not* all query constructs can be applied to non-string fields,
e.g. creating regexp queries, using wildcards/slop/fuzziness will not work.

[[search-dsl-predicate-query-string-other]]
=== Other options

* The score of a `queryString` predicate is variable by default,
but can be <<search-dsl-predicate-common-constantScore,made constant with `.constantScore()`>>.
* The score of a `queryString` predicate can be <<search-dsl-predicate-common-boost,boosted>>,
either on a per-field basis with a call to `.boost(...)` just after `.field(...)`/`.fields(...)`
or for the whole predicate with a call to `.boost(...)` after `.matching(...)`.
* The `queryString` predicate uses the <<mapping-directfieldmapping-search-analyzer,search analyzer>>
of targeted fields to analyze searched text by default,
but this can be <<search-dsl-predicate-common-overriding-analysis,overridden>>.
...
from the example of section <<binding-named-predicate>>.

.Calling a named predicate
====
[source, java, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/mapper/orm/binding/namedpredicate/NamedPredicateIT.java[tags=named-predicate]
----
...
which will be handled as explained in <<binding-named-predicate>>.
====

[[search-dsl-predicate-withparameters]]
== `withParameters`: create predicates accessing query parameters

include::../components/_incubating-warning.adoc[]

The `withParameters` predicate allows building predicates using <<search-dsl-query-parameters,query parameters>>.
...
This type of predicate requires a function that accepts query parameters and returns a predicate.
That function will get called at query building time.

.Creating a predicate with query parameters
====
[source, java, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/predicate/PredicateDslIT.java[tags=with-parameters]
----
