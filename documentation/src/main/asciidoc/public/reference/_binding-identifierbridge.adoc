...
e.g. when the `ValueConvert.PARSE` is used in the matching clause of an <<search-dsl-predicate-id,identifier match predicate>>.
...
----
<1> Use the `ValueConvert.PARSE` and a string representation of the identifier in the <<search-dsl-predicate-id,identifier match predicate>>.
====
