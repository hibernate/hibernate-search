...
* The <<indexing-massindexer,mass indexer>> will fail if you attempt a purge on start (explicitly),
...
Or rely on the <<indexing-massindexer-parameters-purge-all-on-start,`purgeAllOnStart`>>/ <<indexing-massindexer-parameters-drop-and-create-schema,`dropAndCreateSchemaOnStart`>>
defaults to pick the operation available for a particular backend.
