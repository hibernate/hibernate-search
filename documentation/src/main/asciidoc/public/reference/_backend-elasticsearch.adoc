...
There are some limitations applied by Hibernate Search when it comes to using a <<search-dsl-predicate-knn,knn predicate>> with Elasticsearch.
These limitations come from Elasticsearch's integration of k-NN search,
see <<search-dsl-predicate-knn-limitations,this section of the documentation>> for more details.
