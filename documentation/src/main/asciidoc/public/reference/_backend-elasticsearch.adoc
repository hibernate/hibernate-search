...
There are some limitations applied by Hibernate Search when it comes to using a <<search-dsl-predicate-knn,knn predicate>> with OpenSearch.
These limitations come from OpenSearch's feature availability,
see <<mapping-directfieldmapping-vectorSimilarity,this section of the documentation>> for more details.
...
which may change in a future version of Hibernate Search.
...
To change the REST client you need to add the dependency that contains the client
and specify the <<configuration-properties-aggregated-hibernate-search-backend-elasticsearchhibernate.search.backend.client_factory,client factory configuration property>>.
...
. Define a class that implements the `org.hibernate.search.backend.elasticsearch.client.rest4.ElasticsearchHttpClientConfigurer` interface.
...
[[backend-elasticsearch-configuration-http-client-elasticsearch-rest5]]
=== Elasticsearch java client
It is possible to directly configure the underlying Apache HTTP 5 client of the
<<backend-elasticsearch-configuration-client-elasticsearch-client-rest5,Elasticsearch java client>>
...
. Define a class that implements the `org.hibernate.search.backend.elasticsearch.client.rest5.ElasticsearchHttpClientConfigurer` interface.
. Configure Hibernate Search to use that implementation by setting the configuration property
`hibernate.search.backend.client.configurer`
to a <<configuration-bean-reference-parsing,bean reference>> pointing to the implementation,
...
. Define a class that implements the `org.hibernate.search.backend.elasticsearch.client.opensearch.rest.ElasticsearchHttpClientConfigurer` interface.
. Configure Hibernate Search to use that implementation by setting the configuration property
`hibernate.search.backend.client.configurer`
to a <<configuration-bean-reference-parsing,bean reference>> pointing to the implementation,
...
[[backend-elasticsearch-field-types-table]]
...
it needs to determine the <<concepts-entity,entity type>> of each search hit in order to map it back to an entity.

There are multiple strategies to handle this "entity type name resolution",
...
When indexing, the `_entity_type` field is populated transparently with the name of the <<concepts-entity,entity type>> for each document.

When searching, the docvalues for the `_entity_type` field are transparently requested from Elasticsearch
...
* Requires <<indexing-massindexer,full reindexing>> if an <<concepts-entity,entity>> name changes,
even if the index name doesn't change.
...
is used to resolve the index name, and from that the <<concepts-entity,entity type>> name.
