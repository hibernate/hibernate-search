...
<2> Initiate a search query on the index mapped to the `Book` <<concepts-entity,entity>>.
...
by default, the hits of a search query will be <<concepts-entity,entities>> managed by Hibernate ORM,
...
* <<mapping-entitydefinition-loading-selection,configure target entity types to enable loading>>,
...
Though rarely necessary, it is also possible to use <<mapping-entitydefinition-name,entity names>> instead of classes
to designate the <<concepts-entity,entity types>> targeted by the search:
...
[source, JAVA, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/query/QueryDslIT.java[tags=parameters]
----
====

See also:

* <<search-dsl-projection-withparameters,Creating projections with parameters>>
* <<search-dsl-predicate-withparameters,Creating predicates with parameters>>
* <<search-dsl-aggregation-withparameters,Creating aggregations with parameters>>
* <<search-dsl-sort-withparameters,Creating sorts with parameters>>
