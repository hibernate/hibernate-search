...
|java.util.Date|`Instant.ofEpochMilli(long)` as a `java.time.Instant`; see <<mapper-orm-legacy-date-time-apis>> | `Instant.parse(String)`
|java.sql.Timestamp|`Instant.ofEpochMilli(long)` as a `java.time.Instant`; see <<mapper-orm-legacy-date-time-apis>> | `Instant.parse(String)`
|java.sql.Date|`Instant.ofEpochMilli(long)` as a `java.time.Instant`; see <<mapper-orm-legacy-date-time-apis>> | `Instant.parse(String)`
|java.sql.Time|`Instant.ofEpochMilli(long)` as a `java.time.Instant`; see <<mapper-orm-legacy-date-time-apis>> | `Instant.parse(String)`
...
1. Wipe out the existing index,
e.g. by deleting the filesystem directory for Lucene
or using the REST API to delete the index for Elasticsearch.
2. Rebuild the index, for example using the <<mapper-orm-indexing-massindexer,`MassIndexer`>>.
...
Some extra steps may still be needed in the case of Elasticsearch
to update its (internal) mapping;
see the `update` strategy in <<backend-elasticsearch-index-lifecycle>>.
