...
+
Annotation allows to set <<mapper-orm-directfieldmapping-decimalscale, the decimalScale attribute>>.
...
`norms`::
It's only available on `@KeywordField` and `@FullTextField`.
...
`termVector`::
It's only available on `@FullTextField`.
This attribute enables the storing of the term vectors within the documents during indexing.
+
