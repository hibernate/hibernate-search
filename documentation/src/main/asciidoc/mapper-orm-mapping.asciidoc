...
`@ScaledNumberField`::
A numeric field with arbitrary scale and fixed precision.
Only works for `java.math.BigDecimal` fields.
+
Scaled numbers are indexed as integers, typically a long (128 bits), with a fixed scale that is consistent for all values of the field across all documents.
Because scaled numbers are indexed with a fixed precision, they cannot represent all `BigDecimal` values.
...
you be expected to provide "normal" `BigDecimal` values, and Hibernate Search will apply the `decimalScale` and rounding transparently.
+
...
+
A typical value for currency amounts is 2, because only two digits are needed beyond the decimal point.
+
IMPORTANT: Using Hibernate ORM mapping,
