...
`searchable`::
Whenever the field can be searched on.
i.e. whether the field is indexed. So the document will be searchable using this field, using it in the query predicates.
+
...
For both `@FullTextField` and `@KeywordField` fields it is possible to control the creation of the norms.
The norms are index time scoring information for the fields.

Storing them can make scoring queries faster.
But if the score is not used, it is better to not store it.
Since not storing the norms can save a considerable amount of memory.

To store the norms set the attribute `norms` to `Norms.YES`, otherwise set it to `Norms.NO`.
If the attribute is not set or set to `Norms.DEFAULT`,
the storing of norms will depend on the current backend default behavior.
