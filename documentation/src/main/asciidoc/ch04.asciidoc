...
cases this will be as string, but for the types short, int, long, double and float (and their respective
...
being orders of magnitude faster than with the plain string encoding.
...
same scope as `@Field`, but only on properties of numeric type like short, int, long, double and float
...
Lucene supports the numeric types: `Double`, `Long`, `Integer` and `Float`. For properties of type
`Short`, an `Integer` field will be used in the index. Other numeric types
