...
[WARNING]
====
Support for dynamic sharding is deprecated,
and will be removed in Hibernate Search 6.

To implement a `ShardIdentifierProvider` in a future-proof way,
make sure that all the shard identifiers are known on startup,
and that no shard identifiers are added or removed at runtime.
Only then will it be possible to migrate your implementation to Hibernate Search 6.
====
...
Support for the `hibernate.search.[fully qualified entity name].indexName` configuration property is deprecated
and is going to be removed in Hibernate Search 6.
====
...
link:$$http://docs.hibernate.org/orm/current/userguide/html_single/Hibernate_User_Guide.html#multitenacy$$[Hibernate ORM developer's guide].
