...
You just need to call the `fieldTemplate` method on the schema element passed to the binder's `bind(...)` method.

Below is an example of declaring a field template in a property binder.

.Declaring a field template
====
[source, JAVA, indent=0, subs="+callouts"]
----
...
Assuming a field template was declared,
the bridge can then add dynamic fields to the document when indexing,
as shown in the example below.

.Writing to a dynamic field
====
[source, JAVA, indent=0, subs="+callouts"]
----
