...
public class IndexedEmbeddedIncludePathsIT {
	...
	public IndexedEmbeddedIncludePathsIT(BackendConfiguration backendConfiguration) {
	...
	public void smoke() {
		...
		OrmUtils.withinJPATransaction( entityManagerFactory, entityManager -> {
			...
			SubTest.expectException(
					() -> {
						searchSession.scope( Human.class ).predicate()
								.match().onField( "parents.parents.nickname" );
					}
			)
					.assertThrown()
			...
			SubTest.expectException(
					() -> {
						searchSession.scope( Human.class ).predicate()
								.match().onField( "parents.parents.parents.name" );
					}
			)
					.assertThrown()
					.isInstanceOf( SearchException.class )
