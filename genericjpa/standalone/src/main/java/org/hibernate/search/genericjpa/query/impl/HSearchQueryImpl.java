	...
	public List query(EntityProvider entityProvider, Fetch fetchType) {
		...
		if ( fetchType == Fetch.FIND_BY_ID ) {
			ret = projected.stream().map(
					(arr) -> {
						if ( arr[1] == null ) {
							log.trace( "null id in index ommited for query" );
							return null;
						}
						Object obj = entityProvider.get( (Class<?>) arr[0], arr[1], this.hints );
						if ( obj == null ) {
							log.trace( "ommiting object of class " + arr[0] + " and id " + arr[1] + " which was found in the index but not in the database!" );
						}
						return obj;
					}
		...
		if ( ret.size() != projected.size() ) {
			log.trace( "returned size was not equal to projected size" );
		}
		return ret;
	}
