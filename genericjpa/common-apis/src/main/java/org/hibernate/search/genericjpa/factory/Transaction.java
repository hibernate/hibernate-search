...
public class Transaction implements TransactionContext {
	...
	public void commit() {
		if ( !this.progress ) {
			throw new IllegalStateException( "can't commit - No Search org.hibernate.search.genericjpa.factory.Transaction is in Progress!" );
		}
		this.progress = false;
	...
	public void rollback() {
		if ( !this.progress ) {
			throw new IllegalStateException( "can't rollback - No Search org.hibernate.search.genericjpa.factory.Transaction is in Progress!" );
		}
		this.progress = false;
		this.syncs.forEach( Synchronization::beforeCompletion );
