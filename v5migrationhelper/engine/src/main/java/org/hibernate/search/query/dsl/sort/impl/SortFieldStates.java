...
public class SortFieldStates {
	...
	private static final Map<Type, Object> SCALAR_MINIMUMS = new EnumMap<>( Type.class );
	private static final Map<Type, Object> SCALAR_MAXIMUMS = new EnumMap<>( Type.class );
	static {
		initScalarMinMax( Type.DOUBLE, Double.MIN_VALUE, Double.MAX_VALUE );
		initScalarMinMax( Type.FLOAT, Float.MIN_VALUE, Float.MAX_VALUE );
		initScalarMinMax( Type.LONG, Long.MIN_VALUE, Long.MAX_VALUE );
		initScalarMinMax( Type.INT, Integer.MIN_VALUE, Integer.MAX_VALUE );
	}
	...
	public void closeSortField() {
		...
		if ( currentSortFieldNativeSortDescription != null ) {
			...
			sortField = new SortField( null, Type.SCORE, isAsc() );
		}
	...
	private void processMissingValue(SortField sortField) {
		if ( currentMissingValue != null && sortField.getType() != null ) {
			if ( sortField.getType() == Type.STRING || sortField.getType() == Type.STRING_VAL ) {
