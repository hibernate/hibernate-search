...
public class ProcessorIndexedEmbeddedProcessor extends AbstractProcessorAnnotationProcessor {
	...
	public void process(PropertyMappingStep mapping, AnnotationMirror annotation, Element element,
		...
		TypeElement embededType = (TypeElement) context.types().asElement( element.asType() );

		processTypeAndProperties(
				embededType,
				context.programmaticMapping().type( embededType.getQualifiedName().toString() ),
				context
		);
	}
	...
	private String[] toStringArray(AnnotationValue value) {
		if ( value == null ) {
			return EMPTY;
		}
