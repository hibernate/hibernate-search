...
import org.hibernate.search.metamodel.processor.mapping.ProcessorPojoModelsBootstrapIntrospector;
...
public class ProcessorPojoPropertyModel<T> implements PojoPropertyModel<T> {
	...
	public PojoTypeModel<T> typeModel() {
		...
		Optional<Class<?>> loadableType = BuiltInBridgeResolverTypes.loadableType( propertyType );
		if ( loadableType.isPresent() ) {
			return (PojoTypeModel<T>) introspector.typeModel( loadableType.get() );
		}
		else {
			return (PojoTypeModel<T>) introspector.typeModel( (TypeElement) context.typeUtils().asElement( propertyType ) );
		}
	}
