 ...
 * So reuse a FieldCollectorFactory for each field, to create
 * Collector instances as needed.
 * 
 * @author Sanne Grinovero <sanne@hibernate.org> (C) 2011 Red Hat Inc.
 */
public class FieldCacheCollectorFactory {
	
	public static final FieldCacheCollectorFactory CLASS_TYPE_FIELDCACHE_COLLECTOR_FACTORY
		= new FieldCacheCollectorFactory( ProjectionConstants.OBJECT_CLASS,
				FieldCollectorType.STRING, new StringBridge() );
	...
	public FieldCacheCollector createFieldCollector(Collector collector, int totalMaxDocs, int expectedMatchesCount) {
		FieldCacheCollector fieldCollector = createStringFieldCollector(collector, totalMaxDocs, expectedMatchesCount,
	...
	/**
	 * There are two possible implementations of FieldCacheCollector,
	 * one is more efficient for large results, one for small results.
	 ...
	 */
	private FieldCacheCollector createStringFieldCollector(
			Collector collector, int totalMaxDocs, int expectedMatchesCount,
			FieldLoadingStrategy stringDefaultLoader ) {
		...
		if ( expectedMatchesCount > implementationSwitchThreshold ) {
			...
			return new MapFieldCacheCollectorImpl<String>( collector, stringDefaultLoader );
		}
	}
