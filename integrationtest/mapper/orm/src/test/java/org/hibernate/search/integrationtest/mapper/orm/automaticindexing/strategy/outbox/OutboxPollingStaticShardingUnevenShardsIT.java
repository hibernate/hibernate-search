...
/**
 * Test for static sharding with the outbox-polling strategy
...
public class OutboxPollingStaticShardingUnevenShardsIT {
	...
	private SessionFactory setup(Action action, boolean processingEnabled, List<Integer> assignedShardIndices) {
		...
		return ormSetupHelper.start()
				...
				.withProperty( "hibernate.search.automatic_indexing.processing.enabled", Boolean.toString( processingEnabled ) )
				.withProperty( "hibernate.search.automatic_indexing.processing.shards.static", "true" )
				.withProperty( "hibernate.search.automatic_indexing.processing.shards.total_count", TOTAL_SHARD_COUNT )
				.withProperty( "hibernate.search.automatic_indexing.processing.shards.assigned", assignedShardIndices )
