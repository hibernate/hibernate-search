 ...
 *
 * Does not test default bridges, which are tested in {@link MappingDocumentIdDefaultBridgeIT}.
 */
public class MappingDocumentIdBaseIT {
	...
	public void error_unableToResolveDefaultIdentifierBridgeFromSourceType_enumSuperClassWithParameters() {
		@Indexed
		class IndexedEntity {
			Enum<MappingFieldBaseIT.EnumForEnumSuperClassTest> id;
			@DocumentId
			public Enum<MappingFieldBaseIT.EnumForEnumSuperClassTest> getId() {
				return id;
			}
		}
		SubTest.expectException(
				...
				.hasMessageMatching( FailureReportUtils.buildSingleContextFailureReportPattern()
						...
						.failure(
								"Unable to find a default identifier bridge implementation for type 'java.lang.Enum<"
										+ MappingFieldBaseIT.EnumForEnumSuperClassTest.class.getName() + ">'"
						)
						.build()
				);
	}
