...
public abstract class AbstractPojoIndexingPlanOperationContainedNullEntityIT extends AbstractPojoIndexingOperationIT {
	...
	@ParameterizedTest(name = "commit: {0}, refresh: {1}, tenantID: {2}, routing: {3}")
	@MethodSource("params")
	void loadingDoesNotFindEntity(DocumentCommitStrategy commitStrategy, DocumentRefreshStrategy refreshStrategy, String tenantId, MyRoutingBinder routingBinder) {
		setup( commitStrategy, refreshStrategy, tenantId, routingBinder );
	...
	@ParameterizedTest(name = "commit: {0}, refresh: {1}, tenantID: {2}, routing: {3}")
	@MethodSource("params")
	void providedId_providedRoutes(DocumentCommitStrategy commitStrategy, DocumentRefreshStrategy refreshStrategy, String tenantId, MyRoutingBinder routingBinder) {
		setup( commitStrategy, refreshStrategy, tenantId, routingBinder );
		CompletableFuture<?> futureFromBackend = new CompletableFuture<>();
		try ( SearchSession session = createSession() ) {
			SearchIndexingPlan indexingPlan = session.indexingPlan();
	...
	void containingNotIndexed(DocumentCommitStrategy commitStrategy, DocumentRefreshStrategy refreshStrategy, String tenantId, MyRoutingBinder routingBinder) {
		setup( commitStrategy, refreshStrategy, tenantId, routingBinder );
