...
public abstract class AbstractPredicateFieldInObjectFieldIT<V extends AbstractPredicateTestValues<?>>
	...
	public void factoryWithRoot_nested(SimpleMappedIndex<IndexBinding> mainIndex,
		...
		assertThatQuery( mainIndex.query()
				.where( f -> predicateWithRelativePath( f.withRoot( mainIndex.binding().nested.absolutePath ), mainIndex.binding().nested, 0,
						dataSet
				) )
	...
	public void factoryWithRoot_flattened(SimpleMappedIndex<IndexBinding> mainIndex,
			SimpleMappedIndex<MissingFieldIndexBinding> missingFieldIndex, DataSet<?, V> dataSet) {
		assertThatQuery( mainIndex.query()
				.where( f -> predicateWithRelativePath( f.withRoot( mainIndex.binding().flattened.absolutePath ), mainIndex.binding().flattened, 0,
						dataSet
				) )
				.routing( dataSet.routingKey ) )
				.hasDocRefHitsAnyOrder( mainIndex.typeName(), dataSet.docId( 0 ) );
	...
	@MethodSource("params")
	@TestForIssue(jiraKey = "HSEARCH-4162")
	public void inNamedPredicate_nested(SimpleMappedIndex<IndexBinding> mainIndex,
			SimpleMappedIndex<MissingFieldIndexBinding> missingFieldIndex, DataSet<?, V> dataSet) {
		assertThatQuery( mainIndex.query()
				.where( f -> f.named( mainIndex.binding().nested.absolutePath + "." + StubPredicateDefinition.NAME )
						.param( StubPredicateDefinition.IMPL_PARAM_NAME, (PredicateDefinition) context ->
								predicateWithRelativePath( context.predicate(), mainIndex.binding().nested, 0, dataSet )
										.toPredicate() ) )
				.routing( dataSet.routingKey ) )
				.hasDocRefHitsAnyOrder( mainIndex.typeName(), dataSet.docId( 0 ) );
	}
	...
	@MethodSource("params")
	@TestForIssue(jiraKey = "HSEARCH-4162")
	public void inNamedPredicate_flattened(SimpleMappedIndex<IndexBinding> mainIndex,
			SimpleMappedIndex<MissingFieldIndexBinding> missingFieldIndex, DataSet<?, V> dataSet) {
		assertThatQuery( mainIndex.query()
				.where( f -> f.named( mainIndex.binding().flattened.absolutePath + "." + StubPredicateDefinition.NAME )
						.param( StubPredicateDefinition.IMPL_PARAM_NAME, (PredicateDefinition) context ->
								predicateWithRelativePath( context.predicate(), mainIndex.binding().flattened, 0,
										dataSet
								)
										.toPredicate() ) )
				.routing( dataSet.routingKey ) )
