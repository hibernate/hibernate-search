...
class QueryStringPredicateSpecificsIT {
	...
	@PortedFromSearch5(original = "org.hibernate.search.test.dsl.queryStringDSLTest.testqueryString")
	void booleanOperators() {
		...
		Function<String, SearchQuery<DocumentReference>> createQuery = queryString -> scope.query()
				.where( f -> f.queryString().field( absoluteFieldPath ).matching( queryString ) )
				.toQuery();
	...
	void defaultOperator() {
		...
		query = scope.query()
				.where( f -> f.queryString().field( absoluteFieldPath )
	...
	@Test
	@TestForIssue(jiraKey = "HSEARCH-2678")
	@PortedFromSearch5(original = "org.hibernate.search.test.dsl.queryStringDSLTest.testFuzzy")
	void fuzzy() {
		StubMappingScope scope = index.createScope();
		String absoluteFieldPath = index.binding().analyzedStringField1.relativeFieldName;
		Function<String, SearchQuery<DocumentReference>> createQuery = queryString -> scope.query()
	...
	void incompatibleNestedPaths() {
		...
		assertThatThrownBy( () -> index.createScope()
				.predicate().queryString().field( fieldInNestedPath ).field( fieldInRootPath ) )
