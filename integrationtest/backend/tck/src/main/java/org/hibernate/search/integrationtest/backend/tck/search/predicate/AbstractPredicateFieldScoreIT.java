...
public abstract class AbstractPredicateFieldScoreIT<V extends AbstractPredicateTestValues<?>>
	...
	void fieldLevelBoost(SimpleMappedIndex<IndexBinding> index, DataSet<?, V> dataSet) {
		assertThatQuery( index.query()
				.where( f -> f.bool()
						.should( predicate( f, field0Path( index, dataSet ), 0, dataSet ) )
						.should( predicateWithFieldLevelBoost( f, field0Path( index, dataSet ), 42f, 1, dataSet ) ) )
				.routing( dataSet.routingKey ) )
				.hasDocRefHitsExactOrder( index.typeName(), dataSet.docId( 1 ), dataSet.docId( 0 ) );

		assertThatQuery( index.query()
				.where( f -> f.bool()
						.should( predicateWithFieldLevelBoost( f, field0Path( index, dataSet ), 42f,
								0, dataSet
						) )
						.should( predicate( f, field0Path( index, dataSet ), 1, dataSet ) ) )
				.routing( dataSet.routingKey ) )
	...
	void predicateLevelBoost_fieldLevelBoost(SimpleMappedIndex<IndexBinding> index, DataSet<?, V> dataSet) {
		assertThatQuery( index.query()
				.where( f -> f.bool()
						// 2 * 2 => boost x4
						.should( predicateWithFieldLevelBoostAndPredicateLevelBoost( f, field0Path( index, dataSet ), 2f,
								0, 2f, dataSet
						) )
						// 3 * 3 => boost x9
						.should( predicateWithFieldLevelBoostAndPredicateLevelBoost( f, field0Path( index, dataSet ), 3f,
								1, 3f, dataSet
						) ) )
				.routing( dataSet.routingKey ) )
				.hasDocRefHitsExactOrder( index.typeName(), dataSet.docId( 1 ), dataSet.docId( 0 ) );
		...
		assertThatQuery( index.query()
				.where( f -> f.bool()
						// 1 * 3 => boost x3
						.should( predicateWithFieldLevelBoostAndPredicateLevelBoost( f, field0Path( index, dataSet ), 1f,
								0, 3f, dataSet
						) )
						// 0.1 * 3 => boost x0.3
						.should( predicateWithFieldLevelBoostAndPredicateLevelBoost( f, field0Path( index, dataSet ), 0.1f,
								1, 3f, dataSet
						) ) )
				.routing( dataSet.routingKey ) )
				.hasDocRefHitsExactOrder( index.typeName(), dataSet.docId( 0 ), dataSet.docId( 1 ) );
	...
	void predicateLevelBoost_multiFields(SimpleMappedIndex<IndexBinding> index, DataSet<?, V> dataSet) {
		assertThatQuery( index.query()
				.where( f -> f.bool()
						.should( predicateWithPredicateLevelBoost( f, new String[] { field0Path( index, dataSet ), field1Path(
										index, dataSet ) },
								0, 7f, dataSet
						) )
						.should( predicateWithPredicateLevelBoost( f, new String[] { field0Path( index, dataSet ), field1Path(
