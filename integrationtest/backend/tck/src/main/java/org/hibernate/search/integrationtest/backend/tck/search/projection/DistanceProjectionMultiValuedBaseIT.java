...
public class DistanceProjectionMultiValuedBaseIT {
	...
	public void sortable_withoutSort(TestedFieldStructure fieldStructure, DataSet dataSet) {
	...
	@MethodSource("params")
	@TestForIssue(jiraKey = "HSEARCH-3618")
	public void sortable_withSort(TestedFieldStructure fieldStructure, DataSet dataSet) {
		StubMappingScope scope = sortableIndex.createScope();
		...
		ListAssert<Triplet<List<Double>, List<Double>, List<Double>>> hitsAssert = assertThatQuery( scope.query()
				.select( f -> f.composite()
						.from( f.distance( getFieldPath( fieldStructure ), CENTER_POINT_1 ).multi(),
								...
								f.distance( getFieldPath( fieldStructure ), CENTER_POINT_1 ).multi().unit( DistanceUnit.KILOMETERS ) )
						.as( Triplet::new ) )
				.where( f -> f.matchAll() )
				.routing( dataSet.routingKey )
				.toQuery() )
