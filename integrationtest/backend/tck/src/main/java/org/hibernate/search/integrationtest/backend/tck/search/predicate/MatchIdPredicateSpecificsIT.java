...
class MatchIdPredicateSpecificsIT {
	...
	void multiIndex_withIncompatibleIdConverterIndex_dslConverterDisabled() {
		...
		assertThatQuery( scope.query()
				.where( f -> f.id().matching( DOCUMENT_1, PredicateValueConvert.INDEX )
						.matching( INCOMPATIBLE_ID_CONVERTER_DOCUMENT_1, PredicateValueConvert.INDEX ) ) )
				.hasDocRefHitsAnyOrder( b -> {
					b.doc( mainIndex.typeName(), DOCUMENT_1 );
	...
	void parse_unConfigured() {
		...
		assertThatQuery( scope.query()
				.where( f -> f.id().matching( DOCUMENT_1, PredicateValueConvert.STRING ) )
				.toQuery() )
				.hasDocRefHitsAnyOrder( mainIndex.typeName(), DOCUMENT_1 );
	}
	...
	void parse_configured() {
		...
		assertThatQuery( scope.query()
				.where( f -> f.id().matching( ParseIdConverter.id( DOCUMENT_1 ), PredicateValueConvert.INDEX ) )
				.toQuery() )
				.hasDocRefHitsAnyOrder( parseIndex.typeName(), ( ParseIdConverter.id( DOCUMENT_1 ) ) );

		assertThatQuery( scope.query()
				.where( f -> f.id().matching( ParseIdConverter.id( DOCUMENT_1 ), PredicateValueConvert.MAPPING ) )
				.toQuery() )
				.hasDocRefHitsAnyOrder( parseIndex.typeName(), ( ParseIdConverter.id( DOCUMENT_1 ) ) );
	}
