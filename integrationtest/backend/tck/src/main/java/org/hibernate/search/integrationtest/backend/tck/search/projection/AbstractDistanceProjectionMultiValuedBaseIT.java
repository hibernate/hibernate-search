...
abstract class AbstractDistanceProjectionMultiValuedBaseIT {
	...
	void simple(TestedFieldStructure fieldStructure, DataSet dataSet) {
		...
		var projection = distance( scope.projection(), fieldPath, CENTER_POINT_1, "center-param" ).multi().toProjection();
	...
	void duplicated(TestedFieldStructure fieldStructure, DataSet dataSet) {
		...
		var projection = projectionFactory.composite()
				.from(
						distance( projectionFactory, fieldPath, CENTER_POINT_1, "center-param" ).multi(),
						distance( projectionFactory, fieldPath, CENTER_POINT_1, "center-param" ).multi()
				)
	...
	void unit_km(TestedFieldStructure fieldStructure, DataSet dataSet) {
		...
		var projection =
				distance( scope.projection(), getFieldPath( fieldStructure ), CENTER_POINT_1, "center-param" )
						.multi().unit( DistanceUnit.KILOMETERS ).toProjection();
	...
	void unit_miles(TestedFieldStructure fieldStructure, DataSet dataSet) {
		...
		var projection =
				distance( scope.projection(), getFieldPath( fieldStructure ), CENTER_POINT_1, "center-param" )
						.multi().unit( DistanceUnit.MILES ).toProjection();
	...
	void several(TestedFieldStructure fieldStructure, DataSet dataSet) {
		...
		var projection = projectionFactory.composite()
				.from(
						distance( projectionFactory, getFieldPath( fieldStructure ), CENTER_POINT_1, "center-param-1" ).multi(),
						distance( projectionFactory, getFieldPath( fieldStructure ), CENTER_POINT_2, "center-param-2" ).multi(),
	...
	protected abstract DistanceToFieldProjectionValueStep<?, Double> distance(
