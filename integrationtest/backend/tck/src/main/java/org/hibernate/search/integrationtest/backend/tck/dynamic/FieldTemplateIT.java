...
public class FieldTemplateIT {
	...
	public void simple(TestedFieldStructure fieldStructure) {
		...
		index.bulkIndexer()
				...
				.add( DOCUMENT_1, document ->
						initDocument( document, fieldStructure, "foo",
	...
	@MethodSource("params")
	@TestForIssue(jiraKey = "HSEARCH-3273")
	public void matchingPathGlob(TestedFieldStructure fieldStructure) {
		Consumer<IndexSchemaElement> templatesBinder = root -> {
		...
		index.bulkIndexer()
				.add( EMPTY, document -> { } )
				.add( DOCUMENT_1, document ->
						initDocument( document, fieldStructure, "foo_str",
								"matchedValue", "notMatchedValue1", "notMatchedValue2" ) )
				.add( DOCUMENT_2, document ->
						initDocument( document, fieldStructure, "foo_int",
								42, 52, 56 ) )
				.join();
	...
	@MethodSource("params")
	@TestForIssue(jiraKey = "HSEARCH-3273")
	public void matchingPathGlob_precedence_firstDeclared(TestedFieldStructure fieldStructure) {
		Consumer<IndexSchemaElement> templatesBinder = root -> {
			IndexSchemaFieldTemplateOptionsStep<?> step = root.fieldTemplate( "stringTemplate", f -> f.asString() )
		...
		index.bulkIndexer()
				.add( EMPTY, document -> { } )
				.add( DOCUMENT_1, document ->
						initDocument( document, fieldStructure, "foo_str_int",
		...
		assertThatQuery( query(
				f -> f.range().field( getFieldPath( "foo_str_int", fieldStructure ) )
						.between( "3000", "5000" ), fieldStructure )
		)
				.hasDocRefHitsAnyOrder( index.typeName(), DOCUMENT_1 );
		assertThatQuery( query(
				f -> f.range().field( getFieldPath( "foo_int", fieldStructure ) )
						.between( 41, 43 ), fieldStructure )
		)
				.hasDocRefHitsAnyOrder( index.typeName(), DOCUMENT_2 );
	...
	public void parentTemplate_wrongType(TestedFieldStructure fieldStructure) {
		Consumer<IndexSchemaElement> templatesBinder = root -> {
