...
public class NRTWorkspaceImpl extends AbstractWorkspaceImpl implements DirectoryBasedReaderProvider {
	...
	public void setFlushRequirements(boolean flushWrites, boolean flushDeletes) {
		//The second check is put in place to avoid a useless write on the volatile fields
		if ( flushWrites && !this.needFlushWrites ) {
			this.needFlushWrites = true;
		}
		if ( flushDeletes && !this.needFlushDeletes ) {
			this.needFlushDeletes = true;
		}
	}
