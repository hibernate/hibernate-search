...
final class WordDelimiterFilter extends TokenFilter {
	...
	public WordDelimiterFilter(TokenStream in,
		...
		this.iterator = new WordDelimiterIterator(
				charTypeTable, splitOnCaseChange != 0, splitOnNumerics != 0, stemEnglishPossessive != 0
		);
	}
	...
	private void saveState() {
		...
		if ( savedBuffer.length < termAttribute.termLength() ) {
			...
			savedBuffer = new char[oversize( termAttribute.termLength(), 2 )]; //RamUsageEstimator.NUM_BYTES_CHAR
		}
	 ...
	 * @todo Remove this code as soon as we we upgrade to Lucene 3.1 where org.apache.lucene.util.ArrayUtil contains
	 * the oversize method
	 *
	 */
