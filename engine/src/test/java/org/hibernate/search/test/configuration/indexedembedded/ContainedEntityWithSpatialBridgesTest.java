...
public class ContainedEntityWithSpatialBridgesTest {
	...
	public void typeSpatialBridgeNotTriggeringBootstrapException() {
		...
		helper.assertThat( initialCoordinatesQuery ).from( ContainingForTypeSpatialBridge.class ).matchesExactlyIds( 1 );
		helper.assertThat( updatedCoordinatesQuery ).from( ContainingForTypeSpatialBridge.class ).matchesNone();
		...
		helper.assertThat( initialCoordinatesQuery ).from( ContainingForTypeSpatialBridge.class ).matchesNone();
		helper.assertThat( updatedCoordinatesQuery ).from( ContainingForTypeSpatialBridge.class ).matchesExactlyIds( 1 );
	}
	...
	public void propertySpatialBridgeNotTriggeringBootstrapException() {
		...
		helper.assertThat( initialCoordinatesQuery ).from( ContainingForPropertySpatialBridge.class ).matchesExactlyIds( 1 );
		helper.assertThat( updatedCoordinatesQuery ).from( ContainingForPropertySpatialBridge.class ).matchesNone();

		// ... and let's check @ContainedIn works properly
		contained.coordinates = Point.fromDegrees( -12.0, -12.0 );
		helper.add( contained );

		helper.assertThat( initialCoordinatesQuery ).from( ContainingForPropertySpatialBridge.class ).matchesNone();
		helper.assertThat( updatedCoordinatesQuery ).from( ContainingForPropertySpatialBridge.class ).matchesExactlyIds( 1 );
	}
