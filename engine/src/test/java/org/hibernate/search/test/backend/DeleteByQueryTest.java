...
public class DeleteByQueryTest {
	...
	@Rule
	public SearchFactoryHolder factoryHolder = new SearchFactoryHolder(
			Book.class);
	...
	public void testWithWorker() {
		...
		{
			TransactionContextForTest tc = new TransactionContextForTest();
			worker.performWork(new DeleteByQueryWork(Book.class,
					new SingularTermQuery("id", String.valueOf(5))), tc);
			tc.end();
		}
	...
	private void assertCount(int count, ExtendedSearchIntegrator integrator) {
		{
			Query query = integrator.buildQueryBuilder().forEntity(Book.class)
					.get().all().createQuery();
	...
	private List<Work> makeBooks() {
		...
		list.add(new Work(new Book(String.valueOf(5), "Lord of The Rings"),
				WorkType.ADD));
		list.add(new Work(new Book(String.valueOf(6), "The Hobbit"),
				WorkType.ADD));
		return list;
	}
