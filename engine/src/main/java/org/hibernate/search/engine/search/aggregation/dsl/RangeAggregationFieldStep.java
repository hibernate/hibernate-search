...
public interface RangeAggregationFieldStep<PDF extends SearchPredicateFactory> {
	...
	default <F> RangeAggregationRangeStep<?, PDF, F> field(String fieldPath, Class<F> type) {
		return field( fieldPath, type, AggregationInputValueConvert.MAPPING );
	}
	 ...
	 * @return The next step.
	 * @deprecated Use {@link #field(String, Class, AggregationInputValueConvert)} instead.
	 */
	@Deprecated
	default <F> RangeAggregationRangeStep<?, PDF, F> field(String fieldPath, Class<F> type,
			org.hibernate.search.engine.search.common.ValueConvert convert) {
		return field( fieldPath, type,
				org.hibernate.search.engine.search.common.ValueConvert.toAggregationInputValueConvert( convert ) );
	}
	 ...
	 * @param <F> The type of field values.
	 * @param convert Controls how the ranges passed to the next steps and fetched from the backend should be converted.
	 * See {@link AggregationInputValueConvert}.
	 * @return The next step.
	 */
	<F> RangeAggregationRangeStep<?, PDF, F> field(String fieldPath, Class<F> type, AggregationInputValueConvert convert);
