...
public interface RangePredicateFieldStep<
		SR,
		N extends RangePredicateFieldMoreStep<SR, ?, ?>> {
	...
	default <T> RangePredicateFieldMoreGenericStep<SR, ?, ?, RangePredicateFieldReference<? super SR, T>, T> field(
			RangePredicateFieldReference<? super SR, T> fieldReference) {
	 ...
	 * Target the given fields in the range predicate.
	 * <p>
	 * Equivalent to {@link #field(RangePredicateFieldReference)} followed by multiple calls to
	 * {@link RangePredicateFieldMoreGenericStep#field(Object)},
	 * the only difference being that calls to {@link RangePredicateFieldMoreStep#boost(float)}
	 * and other field-specific settings on the returned step will only need to be done once
	 * and will apply to all the fields passed to this method.
	 *
