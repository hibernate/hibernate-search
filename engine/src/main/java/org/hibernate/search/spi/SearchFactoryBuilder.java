...
public class SearchFactoryBuilder {
	...
	public SearchFactoryBuilder currentFactory(SearchIntegrator factory) {
		...
		// This could fail if some fancy framework proxy the object but at this
		// stage they likely won't
		// even proxy SearchIntegrator.
		...
		if (rootFactory == null) {
			if (classes.size() > 0) {
				throw new SearchException(
						"Cannot add a class if the original SearchFactory is not passed");
			}
	...
	private SearchFactoryImplementor buildIncrementalSearchFactory() {
		...
		initDocumentBuilders(searchConfiguration, buildContext,
				searchConfiguration.getProgrammaticMapping());
		final Map<Class<?>, EntityIndexBinding> documentBuildersIndexedEntities = factoryState
				.getIndexBindings();
		Set<Class<?>> indexedClasses = documentBuildersIndexedEntities.keySet();
		for (EntityIndexBinding entityIndexBinding : documentBuildersIndexedEntities
				.values()) {
			// FIXME improve this algorithm to deal with adding new classes to
			// the class hierarchy.
			// Today it seems only safe when a class outside the hierarchy is
			// incrementally added.
			entityIndexBinding.postInitialize(indexedClasses);
		}

		// not really necessary today
		final Map<Class<?>, DocumentBuilderContainedEntity> documentBuildersContainedEntities = factoryState
				.getDocumentBuildersContainedEntities();
		for (DocumentBuilderContainedEntity builder : documentBuildersContainedEntities
				.values()) {
			builder.postInitialize(indexedClasses);
		}

		// update backend
		// TODO make sure the old IndexManagers and backends are disposed - not
		// currently a problem as we only support adding entities incrementally
		SearchFactoryImplementorWithShareableState factory = new ImmutableSearchFactory(
				factoryState);
	...
	private void removeClassesAlreadyManaged() {
		...
		final Map<Class<?>, DocumentBuilderContainedEntity> containedEntities = rootFactory
				.getDocumentBuildersContainedEntities();
		final Map<Class<?>, EntityIndexBinding> indexedEntities = rootFactory
				.getIndexBindings();
		for (Class<?> entity : classes) {
			if (indexedEntities.containsKey(entity)
					|| containedEntities.containsKey(entity)) {
				remove.add(entity);
			}
		}
	...
	private SearchFactoryImplementor buildNewSearchFactory() {
		...
		if (reflectionManager != cfg.getReflectionManager()) {
			cfg = new ReflectionReplacingSearchConfiguration(reflectionManager,
					cfg);
		}

		final SearchMapping mapping = SearchMappingBuilder
				.getSearchMapping(cfg);
		applySearchMappingToMetadata(reflectionManager, mapping);

		factoryState.setSearchMapping(mapping); // might be null if feature is
												// not used

		factoryState.setIndexingStrategy(defineIndexingStrategy(cfg));// need to
																		// be
																		// done
																		// before
																		// the
		...
		QueueingProcessor queueingProcessor = new BatchedQueueingProcessor(
				documentBuildersIndexedEntities, cfg.getProperties());
		...
		factoryState.setCacheBitResultsSize(ConfigurationParseHelper
				.getIntValue(cfg.getProperties(),
						Environment.CACHE_DOCIDRESULTS_SIZE,
	...
	/**
	 * We need to apply the programmatically configured mapping toe the
	 * reflectionManager to fake the annotations.
	 *
	 * @param reflectionManager
	 *            assumed a MetadataProviderInjector, it's state will be changed
	 * @param mapping
	 *            the SearchMapping to apply
	 */
	private void applySearchMappingToMetadata(
			ReflectionManager reflectionManager, SearchMapping mapping) {
		if (mapping != null) {
			...
			injector.setMetadataProvider(new MappingModelMetadataProvider(
					original, mapping));
		}
	}

	private FilterCachingStrategy buildFilterCachingStrategy(
		...
		if (StringHelper.isEmpty(filterCachingStrategyName)
				|| "mru".equalsIgnoreCase(filterCachingStrategyName)) {
			filterCachingStrategy = new MRUFilterCachingStrategy();
		} else {
			Class<?> filterCachingStrategyClass = searchConfiguration
					.getClassLoaderService().classForName(
							filterCachingStrategyName);
			filterCachingStrategy = ClassLoaderHelper.instanceFromClass(
					FilterCachingStrategy.class, filterCachingStrategyClass,
	...
	private void createCleanFactoryState(SearchConfiguration cfg,
		...
		if (rootFactory == null) {
			...
			factoryState
					.setDocumentBuildersIndexedEntities(new ConcurrentHashMap<Class<?>, EntityIndexBinding>());
			factoryState
					.setDocumentBuildersContainedEntities(new ConcurrentHashMap<Class<?>, DocumentBuilderContainedEntity>());
			factoryState
					.setFilterDefinitions(new ConcurrentHashMap<String, FilterDef>());
			...
			factoryState.setServiceManager(new StandardServiceManager(cfg,
					buildContext, Environment.DEFAULT_SERVICES_MAP));
			...
			factoryState
					.setIndexMetadataComplete(cfg.isIndexMetadataComplete());
			factoryState.setTransactionManagerExpected(cfg
	...
	/*
	 * Initialize the document builder This algorithm seems to be safe for
	 * incremental search factories.
	 */
	private void initDocumentBuilders(SearchConfiguration searchConfiguration,
			BuildContext buildContext, SearchMapping searchMapping) {
		ConfigContext configContext = new ConfigContext(searchConfiguration,
				buildContext, searchMapping);

		initProgrammaticAnalyzers(configContext,
				searchConfiguration.getReflectionManager());
		initProgrammaticallyDefinedFilterDef(searchConfiguration
				.getReflectionManager());
		final PolymorphicIndexHierarchy indexingHierarchy = factoryState
				.getIndexHierarchy();
		final Map<Class<?>, EntityIndexBinding> documentBuildersIndexedEntities = factoryState
				.getIndexBindings();
		final Map<Class<?>, DocumentBuilderContainedEntity> documentBuildersContainedEntities = factoryState
		...
		for (Map.Entry<XClass, Class<?>> mapping : classMappings.entrySet()) {
			...
			if (mappedXClass.isAnnotationPresent(Indexed.class)) {
				if (mappedXClass.isAbstract()) {
					log.abstractClassesCannotInsertDocuments(mappedXClass
							.getName());
					continue;
				}
				...
				// FIXME DocumentBuilderIndexedEntity needs to be built by a
				// helper method receiving Class<T> to infer T properly
				// XClass unfortunately is not (yet) genericized: TODO?

				TypeMetadata typeMetadata = metadataProvider
						.getTypeMetadataFor(mappedClass);
				final DocumentBuilderContainedEntity documentBuilder = new DocumentBuilderContainedEntity(
						mappedXClass, typeMetadata,
						searchConfiguration.getReflectionManager(),
						optimizationBlackListedTypes,
						searchConfiguration.getInstanceInitializer());
				// TODO enhance that, I don't like to expose EntityState
				if (documentBuilder.getEntityState() != EntityState.NON_INDEXABLE) {
					documentBuildersContainedEntities.put(mappedClass,
							documentBuilder);
				}
			}
		...
		for (XClass mappedXClass : rootIndexedEntities) {
			...
			MutableEntityIndexBinding entityIndexBinding = indexesFactory
					.buildEntityIndexBinding(mappedXClass, mappedClass,
							searchConfiguration, buildContext);
			...
			// FIXME DocumentBuilderIndexedEntity needs to be built by a helper
			// method receiving Class<T> to infer T properly
			// XClass unfortunately is not (yet) genericized: TODO ?
			TypeMetadata typeMetadata = metadataProvider
					.getTypeMetadataFor(mappedClass);
			final DocumentBuilderIndexedEntity documentBuilder = new DocumentBuilderIndexedEntity(
					mappedXClass, typeMetadata, configContext,
					searchConfiguration.getReflectionManager(),
					optimizationBlackListedTypes,
					searchConfiguration.getInstanceInitializer());
			...
			documentBuildersIndexedEntities
					.put(mappedClass, entityIndexBinding);
		}

		disableBlackListedTypesOptimization(classMappings,
				optimizationBlackListedTypes, documentBuildersIndexedEntities,
	 ...
	 */
	private void disableBlackListedTypesOptimization(
			Map<XClass, Class<?>> classMappings,
		...
		for (XClass xClass : optimizationBlackListX) {
			...
			if (type != null) {
				EntityIndexBinding entityIndexBinding = documentBuildersIndexedEntities
						.get(type);
				if (entityIndexBinding != null) {
					log.tracef(
							"Dirty checking optimizations disabled for class %s",
							type);
					entityIndexBinding.getDocumentBuilder()
							.forceStateInspectionOptimizationsDisabled();
				}
				DocumentBuilderContainedEntity documentBuilderContainedEntity = documentBuildersContainedEntities
						.get(type);
	 ...
	 */
	private static Map<XClass, Class<?>> initializeClassMappings(
			SearchConfiguration cfg, ReflectionManager reflectionManager) {
	...
	private void bindFilterDefs(XClass mappedXClass) {
		FullTextFilterDef defAnn = mappedXClass
				.getAnnotation(FullTextFilterDef.class);
		if (defAnn != null) {
			bindFilterDef(defAnn, mappedXClass);
		}
		FullTextFilterDefs defsAnn = mappedXClass
				.getAnnotation(FullTextFilterDefs.class);
		if (defsAnn != null) {
	...
	private void bindFilterDef(FullTextFilterDef defAnn, XClass mappedXClass) {
		if (factoryState.getFilterDefinitions().containsKey(defAnn.name())) {
			throw new SearchException(
					"Multiple definition of @FullTextFilterDef.name="
							+ defAnn.name() + ": " + mappedXClass.getName());
		}
	...
	private void bindFullTextFilterDef(FullTextFilterDef defAnn) {
		...
		final Map<String, FilterDef> filterDefinition = factoryState
		...
		for (Method method : filterDef.getImpl().getMethods()) {
			...
			if (method.isAnnotationPresent(Key.class)) {
				if (filterDef.getKeyMethod() != null) {
					throw new SearchException("Multiple @Key methods found"
							+ defAnn.name() + ": "
							+ filterDef.getImpl().getName() + "."
							+ method.getName());
				}
			...
			if (name.startsWith("set")
				...
				filterDef.addSetter(
						Introspector.decapitalize(name.substring(3)), method);
			}
		}
		filterDefinition.put(defAnn.name(), filterDef);
	}

	private void initProgrammaticAnalyzers(ConfigContext context,
			ReflectionManager reflectionManager) {
		final Map<?, ?> defaults = reflectionManager.getDefaults();

		if (defaults != null) {
			AnalyzerDef[] defs = (AnalyzerDef[]) defaults
					.get(AnalyzerDefs.class);
			if (defs != null) {
	...
	private void initProgrammaticallyDefinedFilterDef(
			ReflectionManager reflectionManager) {
		Map<?, ?> defaults = reflectionManager.getDefaults();
		FullTextFilterDef[] filterDefs = (FullTextFilterDef[]) defaults
				.get(FullTextFilterDefs.class);
		if (filterDefs != null && filterDefs.length != 0) {
			final Map<String, FilterDef> filterDefinitions = factoryState
					.getFilterDefinitions();
	...
	private static String defineIndexingStrategy(SearchConfiguration cfg) {
		...
		if (!("event".equals(indexingStrategy) || "manual"
				.equals(indexingStrategy))) {
			throw new SearchException(Environment.INDEXING_STRATEGY
	...
	public ErrorHandler createErrorHandler(
			SearchConfiguration searchConfiguration) {
		String errorHandlerClassName = searchConfiguration
				.getProperty(Environment.ERROR_HANDLER);
		if (StringHelper.isEmpty(errorHandlerClassName)) {
			return new LogErrorHandler();
		} else if ("log".equals(errorHandlerClassName.trim())) {
			return new LogErrorHandler();
		} else {
			Class<?> errorHandlerClass = searchConfiguration
					.getClassLoaderService()
					.classForName(errorHandlerClassName);
			return ClassLoaderHelper.instanceFromClass(ErrorHandler.class,
					errorHandlerClass, "Error Handler");
		}
	}

	/**
	 * Implementation of the Hibernate Search SPI WritableBuildContext and
	 * WorkerBuildContext The data is provided by the SearchFactoryState object
	 * associated to SearchFactoryBuilder.
	 */
