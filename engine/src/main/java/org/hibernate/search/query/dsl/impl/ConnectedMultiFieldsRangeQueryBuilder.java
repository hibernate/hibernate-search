...
public class ConnectedMultiFieldsRangeQueryBuilder implements RangeTerminationExcludable {
	...
	private Query createQuery(FieldContext fieldContext, ConversionContext conversionContext) {
		...
		if ( fieldDescriptor != null ) {
			...
			if ( argumentRequiresNumericQuery( rangeContext.getFrom(), rangeContext.getTo() ) ) {
				perFieldQuery = createNumericRangeQuery( fieldName, rangeContext );
			}
			else {
				perFieldQuery = createKeywordRangeQuery( fieldName, rangeContext, queryContext, conversionContext, fieldContext );
	...
	private boolean argumentRequiresNumericQuery(Object from, Object to) {
		if ( from != null ) {
			return argumentRequiresNumericQuery( from );
		}
		else if ( to != null ) {
			return argumentRequiresNumericQuery( to );
		}
		return false;
	}

	private boolean argumentRequiresNumericQuery(Object typedValue) {
		return NumericFieldUtils.isTypeDefaultingToNumericRangeQueries( typedValue );
	}
