...
/**
 * Built-in {@link BridgeProvider} handling spatial index bridging when {@code @Spatial} is involved.
 ...
 */
public class SpatialBridgeProvider implements BridgeProvider {
	...
	public FieldBridge returnFieldBridgeIfMatching(Class<?> returnType, AnnotatedElement bridgedElement, String memberName, ServiceManager serviceManager) {
		...
		if ( bridgedElement.isAnnotationPresent( Spatial.class ) ) {
			Spatial spatialAnn = bridgedElement.getAnnotation( Spatial.class );
			try {
				bridge = buildSpatialBridge( spatialAnn, null, null );
			}
			catch (Exception e) {
				throw LOG.unableToInstantiateSpatial( memberName, e );
			}
			if ( bridge == null ) {
				throw LOG.unableToInstantiateSpatial( memberName, null );
			}
		}
		return bridge;
	}
