...
final class LuceneBackendQueueTask implements Runnable {
	...
	private void applyUpdates() throws InterruptedException, ExecutionException {
		...
		IndexWriterDriver driver = workspace.getIndexWriterDriver( errorContextBuilder );
		if ( driver == null ) {
		...
		try {
			for ( LuceneWork luceneWork : workList ) {
				...
				performWork( luceneWork, resources, driver, monitor );
	...
	static void performWork(final LuceneWork work, final LuceneBackendResources resources, final IndexWriterDriver driver, final IndexingMonitor monitor) {
		work.acceptIndexWorkVisitor( resources.getWorkVisitor(), null ).performWork( work, driver, monitor );
	}
