...
stage('Configure') {
	this.environments = AlternativeMultiMap.create([
			...
			database: [
					...
					// There are some bugs in our integration tests (the tests, not Search itself),
					// and fixes have not been backported to 5.7.
					// Disabling these builds for now.
					new DatabaseITEnvironment(dbName: 'mariadb', mavenProfile: 'ci-mariadb', status: ITEnvironmentStatus.EXPERIMENTAL),
					new DatabaseITEnvironment(dbName: 'postgresql', mavenProfile: 'ci-postgresql', status: ITEnvironmentStatus.EXPERIMENTAL)
			],
			esLocal: [
					new EsLocalITEnvironment(versionRange: '[2.0,2.2)', mavenProfile: 'elasticsearch-2.0', status: ITEnvironmentStatus.SUPPORTED),
					new EsLocalITEnvironment(versionRange: '[2.2,5.0)', mavenProfile: 'elasticsearch-2.2', status: ITEnvironmentStatus.USED_IN_DEFAULT_BUILD)
			]
	])
		...
		executions.put(itEnv.tag, {
			runBuildOnNode {
				helper.withMavenWorkspace {
					resumeFromDefaultBuild()
					mavenNonDefaultIT itEnv, """ \
							clean install -pl org.hibernate:hibernate-search-elasticsearch \
