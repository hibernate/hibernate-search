...
public interface PojoDependencyContext {

	/**
	 * Declare that the given path is read by the bridge at index time to populate the indexed document.
	 *
	 * @param pathFromBridgedElementToUsedValue The path from the bridged element to the value used by the bridge,
	 * as a String. The string is interpreted with default value extractors: see {@link PojoModelPath#parse(String)}.
	 * @return {@code this}, for method chaining.
	 * @throws org.hibernate.search.util.common.SearchException If the given path cannot be applied to the bridged element.
	 * @see #use(PojoModelPathValueNode)
	 */
	default PojoDependencyContext use(String pathFromBridgedElementToUsedValue) {
		return use( PojoModelPath.parse( pathFromBridgedElementToUsedValue ) );
	}
	 ...
	 * @throws org.hibernate.search.util.common.SearchException If the given path cannot be applied to the bridged element.
	 */
	PojoDependencyContext use(PojoModelPathValueNode pathFromBridgedElementToUsedValue);
